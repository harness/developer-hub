...
function focusOnAnchor() {
  ...
  if (element.length > 0) {
    for (let i = 0; i < element.length; i++) {
      ...
      const headingsInDetails = divElement.querySelectorAll('h1, h2, h3 ,h4 , h5');
      element[i].setAttribute('data-collapsed', 'false');
      element[i].setAttribute('open', 'true');
      for (let j = 0; j < headingsInDetails.length; j++) {
        const changedHeading = headingsInDetails[j].id
          .replace(/[^a-zA-Z0-9]/g, '')
          .toLowerCase()
          .replace(/\u200B/g, '')
          .trim();
        const changedTargetDetails = target
          .replace(/[^a-zA-Z0-9]/g, '')
          .toLowerCase()
          .replace(/\u200B/g, '')
          .trim();
        if (changedHeading === changedTargetDetails) {
          ...
          headingsInDetails[j].scrollIntoView();
        }
      }
    }
  }
