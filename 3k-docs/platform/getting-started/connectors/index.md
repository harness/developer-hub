...
A connector encapsulates all the information Harness needs to integrate with an external system, including the endpoint URL, authentication credentials, and connection parameters. Once created, a connector can be referenced by any pipeline, trigger, or platform feature that needs to interact with that system.
