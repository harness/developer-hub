...
sidebar_label: Developer Productivity Agents
sidebar_position: 7
description: AI agents that automate tedious engineering workflows — feature flag cleanup, framework upgrades, and repository onboarding.
---
...
The agent writes four files that are used to create the PR:
...
The Feature Flag Cleanup agent creates draft PRs so your team can review changes before merging. It generates both a PR title and description using AI, making the review easy to understand.
:::
...
The React Upgrade agent automates React version upgrades and code modifications using custom prompts. Despite its name, it's the most flexible agent — it accepts any free-form `prompt` input, making it suitable for framework upgrades, refactoring, dependency updates, and performance optimizations.
...
The React Upgrade agent uses up to 300 AI iterations and creates branches with timestamps (e.g., `react-upgrade-code-agent-1706123456`) to avoid conflicts when running multiple upgrades in parallel.
:::
...
The Onboarding agent has a few notable differences from other agents: it uses **two Claude models** in sequence (Sonnet for fast analysis, Opus for high-quality pipeline generation), it has **no clone step** in the pipeline (the agent container handles cloning itself), and it runs on `linux/amd64` rather than the `arm64` platform used by most agents.
...
The Onboarding agent is ideal for teams migrating to Harness. Import existing GitHub repositories and automatically generate CI pipelines — dramatically reducing the time to get up and running.
:::