...
- **[Insights tab](/docs/infra-as-code-management/workspaces/insights) (Beta):** The workspace **Insights** tab consolidates the actionable findings for a workspace in one place. It surfaces resources that have drifted from your OpenTofu or Terraform configuration and, when Cloud Cost Management is connected, cost optimization opportunities for the workspace resources. Each open insight includes an **Apply with agent** button to remediate it. Contact [Harness Support](mailto:support@harness.io) to request access.

  ![Insights tab for a workspace](./static/insights-tab.png)

- **[IaCM Remediation Agent](/docs/infra-as-code-management/workspaces/remediation-agent) (Beta):** The IaCM Remediation Agent analyzes a drift insight and generates a pull request with recommended IaC changes to reconcile the drift. Launch it with **Apply with agent** from the Insights tab, and Harness runs a project-level remediation pipeline that generates the pull request for you to review and merge. The **Apply with agent** button does not modify your infrastructure directly; the changes take effect only after you merge the pull request. Contact [Harness Support](mailto:support@harness.io) to request access.
...
- **[AWS CDK Phase 2](/docs/infra-as-code-management/iac-provisioners/cdk/overview) (Beta):** CDK provisioner support is expanded with the following additions:
  - **Drift and Destroy steps:** Drift uses the [CloudFormation drift detection API](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-cfn-stack-drift.html) to report resources modified outside of CDK; Destroy deletes the [CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/Welcome.html) stacks managed by a workspace without deleting the workspace itself. 
