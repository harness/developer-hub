...
**GCS and AWS cache key separators prevent pulling incorrect caches from cloned pipelines. (CI-11185, ZD-57012, ZD-57319)**
...
This new feature adds a separator (`/`) to the cache key to prevent accidental prefix matching from cloned pipelines. This feature is enabled by default. If you want to *disable* the separator, add this [stage variable](/docs/platform/pipelines/add-a-stage/#stage-variables): `PLUGIN_DISABLE_SEPARATOR: true`. If you disable the separator, make sure your pipelines [generate unique cache keys](/docs/continuous-integration/use-ci/caching-ci-data/saving-cache#key) to avoid the prefix matching issue.
