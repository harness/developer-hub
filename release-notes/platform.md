...
Invalid user search results were returned when querying from page `2` or higher. We have updated the search functionality to reset the `pageIndex` to `0` after a search query is added or updated, ensuring accurate search results even when the `pageIndex` is `2` or higher. (PL-50907, ZD-62990)

Delegate registration was not failing for inactive accounts. We have added a check during delegate registration to verify account status. Delegates will now fail to register for accounts marked as DELETED or INACTIVE. This item requires Harness Delegate version 24.04.82300. For information about Harness Delegate features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). (PL-48082)
