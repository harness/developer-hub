...
- While using a connector in Project scope during pre-flight check, if the check fails connector reference hyperlinks to Connector URL at account level due to which it routes to a Not Found Page. This was happening because scope details were missing from the api. The issue is fixed now. (CDS-97593, ZD-64673)
- Logs were not being uploaded for shell script and some other steps in case the step timed out. The issue is fixed and shell script now uploads the logs on step timeout. (CDS-97521, ZD-64422)
- Shell script step was not doing a capability check before assigning the tasks to the delegate. The issue is fixed and shell script step will now have host capibility check. (CDS-97512, ZD-66326,66349)
