...
- If you use Kubernetes version 1.16 or later, you can enable the steady state check for Native Helm jobs from Default Settings at any organizational scope (account, organization, or project) in Harness.
  ...
  This item requires Harness Delegate version 23.11.816xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- Rewrote description and asked Pratyush Goel to review, on Slack. Update: Had a Slack convo with Pratyush and the desc now looks final. Just need explicit approval from Pratyush. Waiting. Update: Reviewed and approved in Slack -->
...
- The Identify Service Instance field in the Add New Health Source dialog does not show the plus (`+`) icon when you are using a template to configure a Splunk health source. Consequently, you could not select the service instance after the records were fetched. (CDS-84608, ZD-53584)
  ...
  This issue has been fixed. <!-- Rewrote description and asked Pranesh to review. Check Jira. -->
...
- If you selected the *Expression* value type for a stage or pipeline variable and manually entered the runtime input expression (expressions that begin with `<+input>`, which you typically specify by using the *Runtime input* value type), Harness appended white spaces to the expression when saving the value to YAML. The issue caused the Run Pipeline dialog to not show the input variable. (CDS-83279, ZD-53153)
  ...
  This issue has been fixed. When you manually enter a runtime input expression of the form described earlier, Harness does not append white spaces. <!-- PRNS field had only fix info. Rewrote the description and asked Vikrant Gupta to review. Check Jira. -->
...
- When the width of the dialog in which you enter values for stage variables reduces (for example, when the console view is open), the names of the variables in the dialog get truncated. The issue makes it hard for you to understand what inputs are being requested. (CDS-83225, ZD-53024)
  ...
  Given that such truncation is sometimes unavoidable, Harness has introduced the following changes in the dialog: 
    ...
    - Replaced the description of the variable (in the Description column) with an icon. The icon makes more efficient use of available space and displays the variable's description when you hover over it. <!-- The PRNS field had None in it. Wrote an RN and asked Fardeen Kamal to review/approve. Check Jira. Update: Reviewed and approved by Fardeen in Jira. -->
...
- A stage fails to execute in the following scenario:
  ...
  This issue has been fixed. <!-- The PRNS field had None in it. Wrote an RN and asked Fardeen Kamal to review/approve. Check Jira. Update: Fardeen met me to expedite this. Reviewed and approved in Jira. -->
...
This issue has been fixed.
  ...
  This item requires Harness Delegate version 23.11.816xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- Reviewed and approved by shubhendu.patidar in Slack -->
...
- If the default capacity for the ASG deployment is zero or Null and you choose to create the same number of ASG instances as those that were previously deployed by the pipeline (the `Same as already running Instances` setting), Harness created zero instances. The deployment timed out after waiting for health checks. (CDS-83818)
  ...
  This item requires Harness Delegate version 23.11.816xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate).. <!-- Wrote a description and sent it to Vitalie Safronovici to review over Slack. Awaiting response. Check. Update: Reviewed, updated, and approved. -->
...
- For Rancher-based Kubernetes or Native Helm deployments and instance sync, Harness uses Rancher's `generateKubeconfig` API action. A new kubeconfig token is created on the Rancher cluster each time this API is hit. This led to an accumulation of kubeconfig tokens over time on the Rancher cluster. (CDS-83055, ZD-52924)
  ...
  This item requires Harness Delegate version 23.11.816xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- Clean description in the PRNS field. Made minor edits and asked abhinav.singh2@harness.io to review, in Jira. Check. -->

- The Helm connector's test to check connectivity to an OCI Helm repository in AWS ECR failed with an "Invalid request: Invalid oci url..." error even though the URL to the repository conformed with the formats described in [Connect to an Artifact repository](https://developer.harness.io/docs/platform/connectors/artifact-repositories/connect-to-an-artifact-repo/). The delegate was configured to use a proxy server and the Anonymous authentication type. However, manually fetching Helm charts from the delegate were successful. (CDS-82779, ZD-52343)
  ...
  This item requires Harness Delegate version 23.11.8xxxx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- Rewrote the PRNS description and sent asked for a review in Jira. Check. Also confirm Delegate version because the fix version field is empty. Asked Pratyush this question as well, in the same Jira comment. -->
...
- Earlier, the `tags` API endpoint did not include a timestamp in its response. This shortcoming did not allow Harness to list tags in the lexical order. (CDS-82778)
  ...
  This item requires Harness Delegate version 23.11.816xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- Reworded the PRNS description and asked Sarthak Kasat to review, in Jira. Check. -->
...
- Pipeline executions for WinRM deployments failed intermittently when the deployment was performed by Harness Delegate with version 23.11.81015. The delegate task logs showed that the PowerShell script that was collecting output environment variables failed with exit code 1 while the step logs in the Harness user interface showed that the step succeeded with exit code 0. Certain processes managed by the Windows Remote Management service (namely, `winrshost.exe` with its child process `conhost.exe`) were orphaned and continued to run on the target host. (CDS-82777, ZD-52759, ZD-53411, ZD-53460, ZD-53683)
  ...
  This item requires Harness Delegate version 23.11.816xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- PRNS description insufficient. Added necesary information and sent this description to Alexandru Casian for review, in Jira. Check. -->
...
- Certain Docker registries fail authentication when using the `/v2` endpoint, which is used for health checks in Docker connectors. (CDS-82616, ZD-52513)
  ...
  This item requires Harness Delegate version 23.11.8xxxx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- Clean description in the PRNS field. Made minor edits and asked Deepak Puthraya to review, in Jira. Also, fix version field does not have the delegate version. Asked for that info too. -->
...
- When using the Generic repository format to fetch artifacts from Artifactory, if you used an artifact filter and a non-Regex value for the artifact path, an issue occurred. The issue caused the metadata URL in the service outcome to be incorrect; the URL did not include the repository name. (CDS-82579)
  ...
  This item requires Harness Delegate version 23.10.814xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- This was already added to 814xx. Asked abhishek.abhishek about whether we should drop it from 814xx. Check. -->
...
- Harness did not stop Terraform tasks after you canceled pipeline execution, even if you cancelled execution before the task started to run actual Terraform commands. (CDS-82222, ZD-52603)
  ...
  This item requires Harness Delegate version 23.11.816xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- Clean description in the PRNS field. Made minor edits and asked Tudor Macari to review, in Jira. -->
...
- Currently, the on-premises version of Atlassian BitBucket does not fire push event webhooks when you first push to a new branch. This is inconsistent with other Git providers and also causes Harness's Bitbucket triggers for on-premises repositories to behave inconsistently. (CDS-82110, ZD-52270)
  ...
  This item requires Harness Delegate version 23.11.8xxxx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- Clean description in the PRNS field. Made minor edits and asked vinicius.calasans to review, in Jira. -->
...
- Harness did not honor the working directories specified in script units in the Command steps used in WinRM deployments. Instead, Harness used the default directory configured for the user profile on the target VM. (CDS-82105)
  ...
  This item requires Harness Delegate version 23.11.81405. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- This was already added to 814xx. Asked vitalie safronovici about whether we should drop it from 814xx. Check. -->
...
- The services dashboard did not correctly show primary and canary instances in a Kubernetes deployment. (CDS-81869, ZD-52262, ZD-52930)
  ...
  This item requires Harness Delegate version 23.10.814xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). <!-- This was already added to 814xx. Asked Alexandru Bosii about whether we should drop it from 814xx. Check. -->
