...
- Terraform deployment failed when using AWS connectors (IRSA credential type with assume cross account role) in Terraform steps. When the Terraform Apply command duration exceeded 15 minutes, Terraform output threw an error saying the credentials are invalid when trying to assume a different role from the AWS backend configuration. The default duration for assuming the role in the `aws-java-sdk` is 15 minutes. But the SDK allowed to override it causing the issue. This issue is resolved by introducing a new Harness variable, `HARNESS_AWS_ASSUME_ROLE_DURATION`. When you set this environment variable in Terraform steps, the value mentioned in the variable overrides the default 15 minutes value to assume AWS roles. This item requires Harness Delegate version 01.04.827xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate).(CDS-94355, ZD-60095)
...
- Harness was unable to load the AWS resources during an Amazon ECS Blue Green deployment. The API call for fetching elastic load balancer call was not being made in the stage causing this issue. This issue is fixed now. (CDS-94084, ZD-59734)
...
- Fixed an issue where Harness was unable to integrate Google Cloud Operations with Continuous Verification (CV) for service monitoring. This item requires Harness Delegate version 01.04.827xx. For information about features that require a specific delegate version, go to the [Delegate release notes](/release-notes/delegate). (CDS-93479)
