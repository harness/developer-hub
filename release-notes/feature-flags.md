...
* The Identifier search filter sometimes incorrectly used a wildcard match if two flags had overlapping names, for example, `flag_one` and `flag_one_b`. The detail view of `flag_one_b`, sometimes returned `flag_one` details instead. Because flags order by creation, this only happened when flags were created in a certain order. This fix uses an exact match when searching for flag identifiers.
