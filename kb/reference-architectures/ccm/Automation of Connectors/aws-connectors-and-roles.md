...
module "ccm-member" {
  ...
  external_id             = "harness:${trimspace(each.value.id)}:<your harness account id>"
...
Use the Harness provider to create a CCM connector for each AWS account.  In this example, we are enabling recommendations (OPTIMIZATION), governance (GOVERNANCE), and autostopping (OPTIMIZATION)
