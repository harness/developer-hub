...
The proxy will need security group (network firewall) access to recive traffic on the random ports assigned to different services being proxied by the machine.
...
## Validate traffic can reach the instance
...
You can check that the proxy is running its internal services by SSHing into it and check the system status.
