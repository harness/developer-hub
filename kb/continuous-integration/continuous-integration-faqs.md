...
### What are the requirements for using Save Cache to S3 on AWS?
To use Save Cache to S3, you need an AWS connector with read/write access to a dedicated S3 bucket. Ensure that the IAM roles and policies allow writing to S3, and note that the feature doesn’t work on Windows platforms using AWS cross-account roles due to AWS limitations.

### What should I do if my cache keys conflict after cloning a pipeline?
When cloning a pipeline, cache keys generated by the clone will use the original pipeline's key as a prefix, which may lead to conflicts. To avoid this, you can enable separators in your cache keys by setting the variable PLUGIN_ENABLE_SEPARATOR: true, or ensure that your cloned pipeline generates unique cache keys.
...
### How do I handle port conflicts when running multiple Background steps?
If you're running multiple services, ensure that the host ports do not conflict. You can specify different host ports for each service in the Background step configuration. If the default port is in use, you can change the port binding to avoid conflicts.

### What should I do if my CI pods are being evicted by Kubernetes?
If your CI pods are being evicted due to resource constraints, consider reviewing your resource requests and limits. Ensure they are set appropriately for your pipeline's needs, and troubleshoot potential issues related to build infrastructure. For specific solutions, refer to the CI Knowledge Base for guidance on resource allocation problems.
