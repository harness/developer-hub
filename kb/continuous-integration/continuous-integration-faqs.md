...
In the ‘build and push’ steps used to build Docker images have a context field in them. User can use the context field in the build and push steps, which mounts the current directory at /harness. By copying your files to a specific directory and then mounting it, you can avoid writing secrets into the Docker image layers.
...
If aws connector is configured with the Cross account role ARN, so user need to configure the PLUGIN_USER_ROLE_ARN stage variable as suggested in the below doc:
https://developer.harness.io/docs/continuous-integration/use-ci/build-and-upload-artifacts/build-and-push/build-and-push-to-ecr-step-settings/#aws-connector
...
Yes, you can use the Run step to run your maven command, and upload the test results. There is a field for this in the run step (like in the run tests step). Please see
https://developer.harness.io/docs/continuous-delivery/x-platform-cd-features/cd-steps/containerized-steps/run-step/#report-paths for more information.
