...
No, the delegate is only required when the connector type is selected. The Harness Platform code repo connector handles all operations as long as your repository does not filter any of the IP addresses listed in the [Harness Manager domains and IPs](/docs/platform/references/allowlist-harness-domains-and-ips/#harness-manager).

#### I have a variable named this-var and have assigned a value to this variable. However, when attempting to use the variable, the value always returns null even though I confirmed the value was assigned to the variable through the Harness UI. Why is a null value being returned for my this-var variable?
...
You can also define allowed values by appending the `.allowedValues()` method to `<+input>`. For example: `<+input>.allowedValues(server1, server2)`.  This way, the user can select the server at runtime based on the available values.
...
To correct the error, "There is a new version of this branch," perform the following tasks:
- Refresh the cache and reload the entities from Git by selecting the Reload from Git option.
- A pop-up appears to confirm the reload of the entities from Git. Select **Confirm**.

If these steps still don't fix the issue, try a dummy commit and let the Harness UI update via webhook. Then, once it's updated, you can try and commit anything in the Harness UI, and it should work as expected.

If these steps fail, contact Harness Support and provide them with a HAR file from your browser.
...
A pipeline admin can place approvals as the first step in each stage. This way, the people/groups that need to have the work done for this stage must approve the step for the stage to continue processing its execution.
 
To learn more about adding a manual approval step/stage, go to [Add Harness approval stages](/docs/platform/approvals/adding-harness-approval-stages/).
...
Check the service account being used to sync between Bitbucket and Harness. Bitbucket may have removed the service account's email address. Bitbucket forces users to have an email address, so this will cause an error in Harness if the email address is not present.
