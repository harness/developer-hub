...
### Is there an API to find out who created an environment in a Harness project?
Currently, there is no dedicated API that directly returns the creator of an environment.

### Can I get environment creator details through any existing APIs?
While there’s no direct API, you can use the Audit API to retrieve this information based on audit logs. View Audit API Docs

### Why can’t I use the Audit Trail in the UI for automation?
The Audit Trail in the UI provides the required details but is not ideal for automation or integration use cases, which is why the API approach is preferred.

### What information can the Audit API provide?
The Audit API allows you to filter and retrieve audit events, including actions such as environment creation and the identity of the user who performed them.

### Is there any plan to introduce a direct API for environment metadata like creator info?
As of now, there’s no direct API for this, but you can submit a feature request or check with Harness support for any upcoming enhancements.

### Can Harness audit logs be integrated with Sumo Logic?
Harness supports audit log streaming to destinations like Amazon S3, but Sumo Logic is not natively supported yet. However, integration via S3 is a possible workaround.

### Where can I find the Harness documentation for audit log streaming?
You can refer to the official guide: Audit Streaming Documentation

### Can build or execution logs be streamed from Harness to Sumo Logic?
No, build logs or execution logs are not supported for streaming at this time.

### Can delegate logs be forwarded to Sumo Logic?
Yes, delegate logs can be forwarded, since they are internal to Harness and can be accessed and pushed externally to your logging solution.

### What is the current workaround to send Harness audit logs to Sumo Logic?
Stream audit logs to an S3 bucket, then configure a Sumo Logic AWS S3 source to read those logs.

### Why am I receiving a “S3 objects not readable” error from Sumo Logic?
This error usually means the log files do not meet Sumo Logic’s encoding requirements (typically UTF-8).
Sumo Logic Encoding Reference

### What encoding does Harness use when sending logs to S3?
Harness does not explicitly set encoding, and the logs often appear as ASCII if all characters fall within the ASCII range.

### Can we configure Harness to emit logs in UTF-8 format?
Currently, there is no configuration to change encoding explicitly to UTF-8 when Harness pushes audit logs to S3.

### What is the recommended way to handle encoding issues with Sumo Logic?
Consider writing a pre-processing script (e.g., Lambda) that converts S3 files from ASCII to UTF-8 before ingestion by Sumo Logic.

### Can I use a custom collector or forwarder to pull logs from Harness and push to Sumo Logic?
For delegate logs, yes — you can install a Sumo Logic collector on the delegate host and forward the logs. Audit and build logs do not currently support this.

### What kind of logs are considered “internal” Harness logs?
Delegate logs, task logs, and system events generated by the Harness platform — generally not exposed through standard streaming interfaces.

### Does Harness provide native Sumo Logic integration?
No, Sumo Logic is not natively supported as a log streaming destination in Harness today.

### Is there any plan to support execution log streaming in the future?
While not currently available, you can submit a feature request to the Harness product team for this capability.

### Can I filter which audit events are sent to S3?
Yes, the audit streaming feature allows you to configure filters to control what types of audit events are streamed.

### How often does Harness push logs to the S3 bucket?
Harness pushes logs to S3 at defined intervals, which can be configured during setup — commonly every few minutes.
