...
1. **Create new files** — DMS parent page, merged pages, or restructured skeleton files.
   ...
   **Sidebar configuration for new files:**
   - Module-root files `/docs/[module]/file.md` need explicit entry in `sidebars.ts`
   - Nested files (in subfolders, any depth) only need `sidebar_position` in frontmatter (auto-generate via `_category_.json` or `{ type: 'autogenerated', dirName: '...' }`)
   - Refer to CLAUDE.md "Sidebar Navigation Configuration" section for the full decision tree
