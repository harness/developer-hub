...
If you are new to Ansible, the ideas below build on each other. Later concepts assume you are comfortable with the earlier ones. The documentation follows the same order: read this page, then go to [Get started with Ansible](/docs/infra-as-code-management/configuration-management/ansible/get-started) for the end-to-end walkthrough, then use the [Inventories](/docs/infra-as-code-management/configuration-management/ansible/inventories), [Hosts](/docs/infra-as-code-management/configuration-management/ansible/inventories/hosts), and [Playbooks](/docs/infra-as-code-management/configuration-management/ansible/playbooks) pages as deep dives on each piece.

### 1. Inventories

An **inventory** is the set of machines, called **hosts**, that a playbook run will see, plus the variables attached to them. Everything else in Harness Ansible hangs off an inventory, which is why it is the first thing you create. Harness supports **static** inventories (you define hosts explicitly), **dynamic** inventories (hosts are populated from sources such as a linked Terraform or OpenTofu workspace), and **plugin** inventories (Ansible’s inventory plugins supply hosts from cloud or custom APIs).

Static inventories are the simplest mental model. Dynamic inventories are the natural fit when Terraform or OpenTofu creates the resources you want to configure: host facts and outputs flow forward into Ansible without copy-paste. Dynamic sources read resources from workspace state, so workspaces provisioned through [Terragrunt](/docs/infra-as-code-management/whats-supported) resolve the same way. Plugin inventories help when your organization already standardizes on an Ansible plugin for a given cloud or directory. Go to [Create Ansible inventories](/docs/infra-as-code-management/configuration-management/ansible/inventories) to set up each type.

### 2. Hosts and groups

Ansible runs tasks **against hosts**. Hosts are often grouped (for example `webservers`, `databases`) so a playbook can target a logical tier instead of a long list of machine names. In Harness, you still think in terms of groups and hosts when you read playbook `hosts:` lines; the UI reflects that same model on the inventory’s **Hosts** tab. Go to [Manage hosts and groups](/docs/infra-as-code-management/configuration-management/ansible/inventories/hosts) to add hosts, organize groups, and attach per-host variables.
...
If you come from vanilla Ansible, note the distinction: variables defined in the Harness UI are separate from the `group_vars` and `host_vars` files you may keep in Git. Both work. Go to [Use variable files with Ansible](/docs/infra-as-code-management/configuration-management/ansible/examples/variable-files) to combine repository-based variable files with UI variables.

### 4. Playbooks and Git

A **playbook** is a YAML file (or set of files) that declares ordered tasks and optional handlers. If you have never seen one, go to the [example playbook in Get started](/docs/infra-as-code-management/configuration-management/ansible/get-started#step-3-create-a-playbook) to see the shape before you continue. Harness does not store playbook content; it **reads from your Git repository** at execution time. That preserves pull-request review, branching, and tagging as your source of truth.

Understanding this boundary matters: Harness wires **which repo, branch, and path** to use; Ansible inside your pipeline image still executes the playbook you maintain in Git. Playbooks that depend on Ansible Galaxy collections can install them automatically before execution from a requirements file. Go to [Create Ansible playbooks](/docs/infra-as-code-management/configuration-management/ansible/playbooks) to register playbooks and enable Ansible Galaxy.
