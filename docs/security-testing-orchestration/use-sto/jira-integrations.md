...
2. Create a Jira connector as described in [Connect to Jira](https://developer.harness.io/docs/platform/Resources/Connectors/Ticketing-Systems/connect-to-jira). Note the following:
    * Create your connector at the Account level.
    * Create a [text secret](../../platform/Secrets/2-add-use-text-secrets.md) for your Atlassian API key. You must use the Harness Secret Manager to store this key. Don't use an external secret store, such as Vault.
