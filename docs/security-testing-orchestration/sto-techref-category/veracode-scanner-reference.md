...
Harness recommends you create [text secrets](../../platform/6_Secrets/2-add-use-text-secrets.md) for your authentication credentials — password, API key, API secret key, etc. — and access your secrets using `<+secrets.getValue("<`*`my_secret`*`>")>`.
