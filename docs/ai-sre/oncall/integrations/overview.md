...
AI SRE integrates with the following on-call management platforms:
...
- [xMatters](/docs/ai-sre/oncall/integrations/xmatters): Synchronize on-call groups, schedules, team structures, and contact information.
...
### Migrate from an existing platform
...
<Troubleshoot
  issue="External users not found in Harness after on-call sync"
  mode="docs"
  fallback="Invite users to Harness first, or select them during the Invite Users step of sync. If the wrong user is mapped, ensure email addresses match exactly (case-insensitive), and manually update user mappings after import if needed."
/>

<Troubleshoot
  issue="Schedule time zones or rotation patterns incorrect after on-call sync"
  mode="docs"
  fallback="Verify time zone settings in both systems and manually adjust schedules in AI SRE if automatic conversion fails. Check that the external platform uses supported rotation types; complex custom rotations may need manual recreation."
/>

<Troubleshoot
  issue="Escalation levels missing or notification timeouts different after on-call sync"
  mode="docs"
  fallback="Verify the external platform's escalation policy has all levels populated, since empty levels may be skipped during import. Review timeout settings in both systems, as some platforms use different default values."
/>
...
- Go to [Configure Escalation Policies](/docs/ai-sre/oncall/define-escalation-policies) to configure imported escalation chains.
