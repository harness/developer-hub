...
Escalation policies control what happens when an on-call responder does not acknowledge a page.
...
3. Configure the policy:
   - **Name:** A descriptive name (for example, "Payments Team Escalation").
...
1. The **current on-call responder** (from the attached schedule) is notified through their configured notification channels. If the first channel produces no acknowledgment, AI SRE automatically falls back through the responder's remaining channels before the escalation level times out. Go to [Configure Notification Fallback](/docs/ai-sre/oncall/notification-fallback) to understand the multi-channel fallback order and retry logic.
...
- Go to [Configure Notification Fallback](/docs/ai-sre/oncall/notification-fallback) to control how each responder is notified across channels.
- Go to [Route Alerts](/docs/ai-sre/oncall/configure-alert-rules) to invoke an escalation policy when monitoring alerts fire.
