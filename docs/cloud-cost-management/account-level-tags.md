...
- **CUR 2.0 connector:** Account tags require a **CUR 2.0** connector. Legacy CUR (CUR 1.0) does not expose the `resource_tags` field that Harness uses to ingest account-level tags. Go to [Set up cost visibility for AWS](/docs/cloud-cost-management/provider-integrations/cloud-providers/aws) to configure or migrate your connector to CUR 2.0.
