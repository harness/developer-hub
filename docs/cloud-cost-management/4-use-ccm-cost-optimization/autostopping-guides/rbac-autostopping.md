...
- `CCM Viewer` : Grants permission to view CACM entities, including AutoStopping rules and loadbalancers as whole.
- `CCM Admin`: Grants permission to create/edit/delete CACM entities, including all AutoStopping rules and loadbalancers.
...
Example:
If a user is assigned the `CACM Viewer` role on an RG called `rg_dev_connectors`, which includes a connector named `dev_connector` (linked to a DEV cloud account), then:
...
2. Select all the **CACM AWS Account Connectors** for the cloud accounts you want to grant access to.  
