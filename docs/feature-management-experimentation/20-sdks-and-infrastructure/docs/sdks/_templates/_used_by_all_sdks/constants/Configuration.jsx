...
export const HTTPS_AUTHENTICATOR                         = () => <> If set, used by the {props.sdk ?? 'SDK'} to authenticate network requests. Value must be an implementation
...
export const MOBILE_BACKGROUND_SYNC_WHEN_BATTERY_NOT_LOW = () => <> When set to <code>true</code>, the {props.sdk ?? 'SDK'} synchronizes in the background only if battery
                                                                    level is not low. </>;
export const MOBILE_BACKGROUND_SYNC_WHEN_WIFI_ONLY       = () => <> When set to <code>true</code>, the {props.sdk ?? 'SDK'} synchronizes in the background only if the
                                                                    wifi connection is available (unmetered). When <code>false</code>, the {props.sdk ?? 'SDK'} synchronizes
...
export const MOBILE_PROXY_HOST                           = () => <> The location of the proxy using standard URI:&nbsp;
                                                                    <code>scheme://user:password@domain:port/path</code>. If no port is provided, the {props.sdk ?? 'SDK'}
                                                                    defaults to port 80. </>;
...
export const MOBILE_STORAGE_PREFIX                       = () => <> If set, the prefix will be prepended to the database name used by the {props.sdk ?? 'SDK'}. </>;
export const SCHEDULER_EVENTS_PUSH_RATE                  = () => <> Time (seconds) interval for the {props.sdk ?? 'SDK'} to send events to Harness servers. </>;
export const SCHEDULER_EVENTS_QUEUE_SIZE                 = () => <> Maximum number of events to queue. When the queue is full, the {props.sdk ?? 'SDK'} flushes events and
                                                                    resets a timer. </>;
export const SCHEDULER_FEATURES_REFRESH_RATE             = () => <> Time (seconds) interval for the {props.sdk ?? 'SDK'} to poll Harness servers for changes to feature
                                                                    flags. </>;
export const SCHEDULER_IMPRESSIONS_REFRESH_RATE          = () => <> Time (seconds) interval for the {props.sdk ?? 'SDK'} to send impressions to Harness servers. </>;
export const SCHEDULER_IMPRESSIONS_QUEUE_SIZE            = () => <> Maximum number of impressions (decisioning events) to queue. When the queue is full, the
                                                                    {props.sdk ?? 'SDK'} flushes impressions and resets a timer. </>;
export const SCHEDULER_LOCALHOST_REFRESH_RATE            = () => <> In localhost mode, time (seconds) interval to sync feature flags and segments (mocked
                                                                    ...
                                                                    in localhost mode. </>;
export const SCHEDULER_PERSISTENT_CACHE_ENABLED          = () => <> The {props.sdk ?? 'SDK'} polls Split servers for changes to segments at this rate (in seconds). </>;
export const SCHEDULER_SEGMENTS_REFRESH_RATE             = () => <> Time (seconds) interval for the {props.sdk ?? 'SDK'} to poll Harness servers for changes to
                                                                    segments. </>;
export const SCHEDULER_TELEMETRY_REFRESH_RATE            = () => <> Time (seconds) interval for the {props.sdk ?? 'SDK'} to send diagnostic data to Harness servers. </>;
export const STARTUP_EVENTS_FIRST_PUSH_WINDOW            = () => <> Time (seconds) for the first push of events after {props.sdk ?? 'SDK'} initialization. </>;
...
export const STARTUP_REQUEST_TIMEOUT_BEFORE_READY        = () => <> Time (seconds) to wait for a response from the /splitChanges and /mySegments HTTP API
                                                                    endpoints during {props.sdk ?? 'SDK'} initialization. </>;
export const STARTUP_RETRIES_ON_FAILURES_BEFORE_READY    = () => <> Number of retries when sending requests to the /splitChanges and /mySegments HTTP API
                                                                    endpoints during {props.sdk ?? 'SDK'} initialization. </>;
export const STORAGE_TYPE                                = () => <> Storage type used by the {props.sdk ?? 'SDK'}. Possible values are <code>MEMORY</code> and&nbsp;
                                                                    <code>LOCALSTORAGE</code>. </>;
export const STREAMING_ENABLED                           = () => <> Enable the streaming service for synchronization of feature flag and segment data.
                                                                    (If there is an issue with streaming, the {props.sdk ?? 'SDK'} falls back to the polling service.)
                                                                    If <code>false</code>, the {props.sdk ?? 'SDK'} uses polling only. </>;
export const SYNC_CONFIG                                 = () => <> Optional <code>SyncConfig</code> instance, used to filter feature flags to be synced and
                                                                    evaluated by the {props.sdk ?? 'SDK'}. These filters can be created with the&nbsp;
                                                                    ...
                                                                    (for the given traffic type) are downloaded by the {props.sdk ?? 'SDK'}. </>;
export const SYNC_ENABLED                                = () => <> Enable periodically updating feature flags and segments definitions. When&nbsp;
                                                                    <code>false</code>, the {props.sdk ?? 'SDK'} downloads data from the Harness only upon init. Setting
...
export const SYNC_IMPRESSIONS_MODE                       = () => <> Defines types of impressions (decisioning events) sent to Harness servers. Possible
                                                                    ...
                                                                    <ul>
                                                                        ...
                                                                        <li><code>NONE</code>: no impressions (the {props.sdk ?? 'SDK'} tracks only minimum viable data to
...
export const SYNC_REQUEST_OPTIONS_GET_HEADER_OVERRIDES   = () => <> Callback function that can be used to override the Authentication header or append
                                                                    new headers to the {props.sdk ?? 'SDK'}'s HTTP(S) requests. </>;
export const SYNC_SPLIT_FILTERS                          = () => <> Filter the feature flags to be synced and evaluated by the {props.sdk ?? 'SDK'}. This configuration
                                                                    ...
                                                                    are downloaded by the {props.sdk ?? 'SDK'}. </>;
...
export const WEB_LOG_LEVEL                               = () => <> Boolean flag, string log level, or logger instance for activating {props.sdk ?? 'SDK'} logs. See&nbsp;
                                                                    <a href='../Logging'>Logging</a> for details. </>;
export const WEB_STORAGE                                 = () => <> Pluggable storage instance to be used by the {props.sdk ?? 'SDK'} as a complement to in memory storage.
                                                                    Currently supported value is <code>InLocalStorage</code>. See&nbsp;
                                                                    <a href="#configure-localstorage-cache-for-the-sdk">Configure LocalStorage cache for the {props.sdk ?? 'SDK'}</a>&nbsp;
                                                                    for details. </>;
