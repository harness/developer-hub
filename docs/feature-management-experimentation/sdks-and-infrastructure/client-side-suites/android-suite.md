...
| impressionsRefreshRate | Controls how frequently the impressions cache expires after a write (in seconds). The treatment log captures which customer saw which treatment (on, off, etc.) and is periodically flushed back to Harness servers. | 1800 seconds |
...
Event metadata is supported in the Android Suite version 2.2.0 or later. 
:::
...
For required hashes and migration guidance, see [Certificate Pinning](/docs/feature-management-experimentation/sdks-and-infrastructure/client-side-suites/#certificate-pinning).
:::
