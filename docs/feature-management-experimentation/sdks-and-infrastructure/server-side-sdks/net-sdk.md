...
| --- | --- | --- |
| FeaturesRefreshRate | The SDK polls Harness servers for changes to feature flags at this rate (in seconds). | 60 seconds |
| SegmentsRefreshRate | The SDK polls Harness servers for changes to segments at this rate (in seconds). | 60 seconds |
| ImpressionsRefreshRate | The treatment log captures which customer saw what treatment (on, off, etc) at what time. This log is periodically flushed back to Harness servers. This configuration controls how quickly the cache expires after a write (in seconds). | 300 seconds |
| MaxImpressionsLogSize | The SDK queues impressions locally before sending them to the Harness servers. This setting controls how many impressions are stored locally before sending them to the Harness servers. | 30000 |
...
| EventsQueueSize | The SDK collects the events generated by the customer. This setting controls how many events are stored locally before sending them to the Harness servers (for standalone mode only). | 10000 |
