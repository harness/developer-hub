import eventTypes from '@site/docs/feature-management-experimentation/50-release-monitoring/static/events/received-events.png';
...
:::note
Other SDK languages each have specific syntax that need to be followed on their own help pages. Refer to our [SDK documentation](/docs/feature-management-experimentation/sdks-and-infrastructure) for more information about using the `track` call for your selected SDK.
:::
...
To help verify that events are being received by Harness FME, from the left navigation pane, click the **profile button** at the bottom, select **Admin settings**, and click **Event types**. There you can see event types and a stream of events per event type.
