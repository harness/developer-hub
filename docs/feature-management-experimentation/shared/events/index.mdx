import eventTypes from '@site/docs/feature-management-experimentation/release-monitoring/static/events/event-types.png';
import dataHub from '@site/docs/feature-management-experimentation/release-monitoring/static/events/data-hub.png';
...
You can view the event properties being sent for a given event type using the [Live tail functionality in the Data Hub](/docs/feature-management-experimentation/feature-management/monitoring-analysis/live-tail).
...
1. The **Explore** tab enables you to visualize the event volume trend for a selected event type over the last 7 days. To visualize your events, first select a traffic type and an event type on the form to the left. After this, click on the **Query** button to generate a graph of your results.

   <img src={dataHub} alt="Applying Metric Filters" width="700" />

1. The [**Live Tail** tab](/docs/feature-management-experimentation/feature-management/monitoring-analysis/live-tail) enables you to run a live query of impressions and events, so you can understand your event stream in more detail, including which event properties are being sent with each event type.