...
1. Locate the Amazon S3 integration and click **Add**.
1. Select the project for which you would like to configure the integration. You can also click **Warehouse** under `Categories` to filter to Amazon S3.
1. In the `Send events to Split` field, click **Add configuration**.
1. In the `Select environment` field, select the Harness FME environments that receive S3 data.
1. In the `Paste S3 bucket name` field, provide the name of your unique S3 bucket from above. (The directory will be created if it does not exist.)
1. In the `Paste SNS topic ARN` field, provide the SNS topic ARN from above.
1. In the `Paste status folder location` field, provide the file path for status files.

   ![](./static/amazon-s3-integration.png)

1. Click **Save**.
1. Navigate back to your Amazon S3 integration. Harness FME has generated the Notification endpoint URL and Role ARN fields.

### Update S3 bucket policy for Harness FME access (inbound)

1. In the Amazon S3 console, navigate to your S3 bucket.
1. In the `Edit bucket policy` field, select **Bucket policy** and replace the JSON with the following snippet. Update the Role ARN with the value generated by Harness FME as well as your bucket name as appropriate.
...
1. Locate the Amazon S3 integration and click **Add**.
1. Select the project for which you would like to configure the integration. You can also click **Warehouse** under `Categories` to filter to Amazon S3.
1. In the `Send impressions to S3` section, click **Add configuration**.
1. In the `Select environment` field, select the Harness FME environments that receive S3 data.
1. In the `Paste S3 bucket name` field, enter the name of your unique S3 bucket name.

   ![](./static/amazon-s3-outbound-s3-split-ui.png)

1. In the `Provide the write location in your S3 bucket` field, optionally put the S3 prefix you’d like the impressions files to be saved to. If no value is provided, files are saved to the root directory of your provided S3 bucket. 
1. In the `Select file format` field, select the desired format for your impressions files.
1. In the `Select file compression type` field, select the desired file compression format for your impression files.
 
   :::info[Note]
   The list only displays compression methods that are compatible with your chosen file format.
   :::

1. Click **Save**. Note your role ARN generated by Harness FME.
