...
Now that your SDK is initialized, let's fetch a [**Feature Gate**](/docs/feature-management-experimentation/get-started-with-fme/key-concepts/environments). Feature Gates can be used to create logic branches in code that can be rolled out to different users from the Statsig Console. Gates are always **CLOSED** or **OFF** (think `return false;`) by default.
...
Then we have **Layers/Experiments**, which you can use to run A/B/n experiments. We offer two APIs, but we recommend the use of [layers](/docs/feature-flag-management-experimentation/get-started-with-fme/key-concepts/environments) to enable quicker iterations with parameter reuse.
...
See the guide on [device level experiments](/docs/feature-management-experimentation/get-started-with-fme/key-concepts/metrics)
