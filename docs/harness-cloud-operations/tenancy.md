...
1. Harness and customer will ensure that the account has a vanity URL. If no such URL exists, then Harness will first provision a vanity URL in consultation with the customer. Follow the steps here to [setup vanity URL] (/docs/platform/authentication/authentication-overview/#set-up-vanity-url).
...
5. Harness will notify the customer that migration is complete. At this time, customer will be able to access the UI and initiate regular usage.
