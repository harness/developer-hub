...
## Auto-create services and environments

When importing Argo CD projects through a [Bring Your Own Argo CD (BYOA) setup](/docs/continuous-delivery/gitops/connect-and-manage/multiple-argo-to-single-harness), Harness can automatically create services and environments in the target Harness project based on the `harness.io/serviceRef` and `harness.io/envRef` labels defined in your ApplicationSet template.
...
<div style={{textAlign: 'center'}}>
  <DocImage path={require('./static/map-argo-project.png')} width="60%" height="60%" title="Click to view full size image" />
</div>
...
<div style={{textAlign: 'center'}}>
  <DocImage path={require('./static/importing-projects.png')} width="60%" height="60%" title="Click to view full size image" />
</div>
