...
**New to ApplicationSets and PR Pipelines?** See the [ApplicationSet and PR Pipeline Summary](#applicationset-and-pr-pipeline-summary) below.
...
You often here the term wave deployments used when PR Pipelines are discussed.
...
With Harness GitOps, you can implement wave deployments by creating multiple environments for your application, one for each cloud region, and gradually promoting changes from one environment to the next. This way, you can test changes in a safe and controlled manner before releasing them to the entire user base.
