...
The default `allowCrossGenerationAccess` value for existing CD accounts in Harness is `TRUE`, but it is set to `FALSE` for for non-CD users. For all new FirstGen and NextGen user accounts, this value is set to `FALSE` by default.
