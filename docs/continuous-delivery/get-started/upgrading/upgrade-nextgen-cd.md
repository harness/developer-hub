...
The default `allowCrossGenerationAccess` value for accounts with `defaultExperience` as NextGen is `TRUE`. For new FirstGen and NextGen user accounts, this value is set to `FALSE` by default.
