...
5. When scaling the Echo service while the HA setting is enabled, 1 scheduler and multiple workers will need to be run. The worker service can then be independently scaled. To utilize CRON scheduling using Echo, SQL would need to be switched on so it has a lock on the Database. It should be noted that if the CRON scheduler, for some reason could not trigger a pipeline, then a manual intervention or a single running Echo that’s designed to trigger any previously failed CRON jobs would be needed.
