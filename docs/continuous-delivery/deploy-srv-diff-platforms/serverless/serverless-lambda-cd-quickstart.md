...
<details>
<summary>Explanation and Pipeline YAML</summary>
...
The Serverless Framework uses these runtime-specific Docker images to:
- Create a local development environment that closely matches the AWS Lambda execution environment
- Build Lambda functions with the correct dependencies and compilation settings
- Package applications with runtime-specific optimizations
...
1. Navigate to the **ServerlessV2CombineAndPush** pipeline in Harness
2. Click **Run Pipeline**
3. Enter the required parameters:
   - **VERSION**: Version number for your plugin (e.g., `1.1.0-beta`)
...
- **Version Tagging**: The VERSION variable corresponds to specific code changes in the repository. With each new code change, we push a new tag and publish new Docker images with these tags, allowing users to access specific versions of the plugin.

<details>
<summary>Validating Image Compatibility</summary>
...
```bash
# Check runtime image libraries
docker run --rm public.ecr.aws/sam/build-java21:1.140.0-20250605234711-x86_64 bash -c "ls -la /lib64/libstdc++.so*"

# Check Node.js image libraries
docker run --rm public.ecr.aws/sam/build-nodejs22.x:1.140.0-20250605234713-x86_64 bash -c "ls -la /lib64/libstdc++.so*"
```

**Compatible**: Both images show the same version (e.g., `libstdc++.so.6.0.33`)  
**Incompatible**: Different versions may cause runtime errors

#### Common Compatibility Issues

1. **Missing Libraries**: If the runtime image lacks libraries required by Node.js
2. **Version Conflicts**: Different versions of shared libraries between images
3. **Path Configuration**: Incorrect PATH environment variable settings
4. **Permission Issues**: Insufficient permissions for Node.js executables
