...
sidebar_label: WinRM Deployments
...
For WinRM, you can access artifacts from [Harness Artifact Registry](/docs/continuous-delivery/x-platform-cd-features/services/artifact-sources#harness-artifact-registry) (**Docker**, **Generic**, **NPM**, **NuGet**, **Maven**, and **Raw** artifact types), **Jenkins**, **Artifactory**, **Custom**, **Nexus**, and **Amazon S3**.
...
Enable [Harness File Store](/docs/continuous-delivery/x-platform-cd-features/services/add-inline-manifests-using-file-store) to upload config files from the file store.
...
- **Harness key concepts**: Go to [Harness key concepts](/docs/platform/get-started/key-concepts) to establish a general understanding of Harness.
...
- **OIDC-enabled AWS connector (optional)**: Harness supports WinRM with an **OIDC-enabled AWS connector**, but it requires delegate version `854xx` or later. Go to [AWS OIDC connector reference](/docs/platform/connectors/cloud-providers/ref-cloud-providers/aws-connector-settings-reference) to review the requirements.
...
<div align="center">
  <DocImage path={require('./static/win-rm-tutorial-128.png')} alt="Select WinRM as the deployment type and set up the stage" width="80%" title="Click to view full size image" />
</div>
...
<div align="center">
  <DocImage path={require('./static/win-rm-tutorial-129.png')} alt="Configure anonymous authentication for the Artifactory connector" width="80%" title="Click to view full size image" />
</div>
...
:::note
The artifact [expression](/docs/continuous-delivery/x-platform-cd-features/services/artifact-sources) may vary based on the chosen artifact type.
:::
...
<div align="center">
  <DocImage path={require('./static/pdc-single-host.png')} alt="Single Host configuration mode for a PDC infrastructure" width="80%" title="Click to view full size image" />
</div>
...
<div align="center">
  <DocImage path={require('./static/pdc-specify-host.png')} alt="Specify host attributes and match criteria for filtering" width="80%" title="Click to view full size image" />
</div>
...
<div align="center">
  <DocImage path={require('./static/win-rm-tutorial-130.png')} alt="Name the new PDC connector" width="80%" title="Click to view full size image" />
</div>
...
<div align="center">
  <DocImage path={require('./static/win-rm-tutorial-131.png')} alt="Enter the EC2 host name for the PDC connector" width="80%" title="Click to view full size image" />
</div>
...
<div align="center">
  <DocImage path={require('./static/win-rm-tutorial-132.png')} alt="Successful PDC host connection test" width="80%" title="Click to view full size image" />
</div>
...
<div align="center">
  <DocImage path={require('./static/pdc-host-groups.png')} alt="Host Groups tab in the Infrastructure Definition dialog" width="80%" title="Click to view full size image" />
</div>
...
- Terraform:
  ...
  - [Terraform Rollback](/docs/continuous-delivery/cd-infrastructure/terraform-infra/rollback-provisioned-infra-with-the-terraform-rollback-step). To display the Terraform Rollback step, toggle the **Rollback** setting.
...
- CloudFormation:
  ...
  - [Rollback Stack](/docs/continuous-delivery/cd-infrastructure/cloudformation-infra/rollback-provisioned-infra-with-the-cloud-formation-rollback-step). To display the Rollback Stack step, toggle the **Rollback** setting.
...
<div align="center">
  <DocImage path={require('./static/8722541e819fd752abc35693bd00e38cca2bce5df264afa89cbf61288fbc0604.png')} alt="Map provisioner outputs to Host Array Path and Host Data Mapping settings" width="80%" title="Click to view full size image" />
</div>
...
<div align="center">
  <DocImage path={require('./static/8722541e819fd752abc35693bd00e38cca2bce5df264afa89cbf61288fbc0604.png')} alt="Map Terraform outputs to Infrastructure Definition settings using provisioner expressions" width="80%" title="Click to view full size image" />
</div>
...
<Troubleshoot
  issue="WinRM sessions are not being reused and Harness keeps creating new shared sessions on the delegate"
  mode="docs"
  fallback="Confirm the CDS_SHARED_SESSION_WINRM_NTLM_NG feature flag is enabled, verify you use NTLM authentication (Kerberos is not pooled), ensure the host and credential are identical so sessions share the same group, and review delegate logs for POOL_HIT and POOL_MISS messages (visible only with debug logging enabled)."
/>

<Troubleshoot
  issue="Harness reports WinRM session pool capacity warnings because the session pool is full"
  mode="docs"
  fallback="Increase WINRM_SESSION_POOL_MAX_SESSIONS, reduce WINRM_SESSION_POOL_IDLE_TIMEOUT_SECONDS (kept higher than WINRM_SESSION_POOL_CLEANUP_INTERVAL_SECONDS) to free capacity faster, and resolve any persistent connection errors that prevent sessions from returning to the pool."
/>

<Troubleshoot
  issue="Increased WinRM connection errors occur after enabling session reuse on the Harness delegate"
  mode="docs"
  fallback="Increase the MaxShellsPerUser limit on target hosts, allow persistent WinRM connections through your firewall, and decrease WINRM_SESSION_POOL_IDLE_TIMEOUT_SECONDS if the network terminates idle connections prematurely."
/>
...
Go to [Selective rerun and skipping hosts with the same artifact](/docs/continuous-delivery/deploy-srv-diff-platforms/traditional/ssh-ng/#selective-rerun-and-skipping-hosts-with-same-artifact) to configure selective reruns.
...
- [Provisioning overview](/docs/continuous-delivery/cd-infrastructure/provisioning-overview): Dynamically provision your target infrastructure during deployment.
- [Harness variables and expressions](/docs/platform/variables-and-expressions/harness-variables): Reference hosts and instance properties in your steps.
