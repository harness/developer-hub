...
The AWS SAM CLI uses these runtime-specific Docker images to:
...
1. Navigate to the **samCombineAndPush** pipeline in Harness
                        ...
                        spec:
                          ...
                          command: |-
                            ...
                            DOCKEREOF
                                ...
                                if buildah build --isolation=chroot --storage-driver=vfs --file /tmp/Dockerfile --build-arg DOCKER_VERSION="26.0.1" --tag "${FINAL_IMAGE}" /tmp 2>/dev/null; then
                            ...
                            if command -v skopeo >/dev/null 2>&1; then
                                ...
                                if skopeo copy \
                                    ...
                                    echo ""
                                    echo " IMPORTANT NOTE:"
                                    echo "This is a temporary workaround. The final image contains:"
                                    echo "- Your compiled SAM plugin binary"
                                    echo "- Missing SAM CLI environment (from ${SAM_BASE_IMAGE})"
                                    echo ""
                                    echo "To get the full combined image, you'll need:"
                                    echo "1. A less restrictive Kubernetes environment, or"
                                    echo "2. Use Harness CI stage with BuildAndPushDockerRegistry step, or"
                                    echo "3. Build externally (e.g., Google Cloud Build) and push to registry"
                                    echo ""
...
- **About the Warning Message**: If you see a message about "Missing SAM CLI environment," this is **misleading**. The scratch image already contains everything needed for the plugin to function properly. This warning appears when the pipeline falls back to the skopeo method, but it does not indicate a functional problem with the resulting image.

- **Infrastructure Agnostic Design**: The pipeline is specifically designed to work across different infrastructure types, with multiple fallback strategies to ensure success regardless of environment restrictions.

- **Base Image Compatibility**: Only official AWS SAM build images from the ECR Gallery are tested and supported. Using alternative base images may result in missing dependencies or runtime errors.
