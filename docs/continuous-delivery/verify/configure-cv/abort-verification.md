...
description: Learn how to abort a running AI Verify (v1) step.
sidebar_position: 30
---
...
Users who have the required permissions can abort a running AI Verify (v1) step. The `Abort Verification` command becomes available only after verification has begun, and it includes the following options:
- Mark as success
- Mark as failure

The Abort Verification command stops verification and assigns the specified success or failure state to the AI Verify (v1) step. Harness then proceeds to the next step or stage in the pipeline.
...
- Wait until the AI Verify (v1) step has begun (the step status changes from `Not Started` to `In Progress`).
