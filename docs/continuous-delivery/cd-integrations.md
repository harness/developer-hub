...
| Flag | Description |
| --- | --- |
| `PIPE_ENABLE_GITHUB_ENTERPRISE_PROVIDER_FLOW` | Enables users to integrate their GitHub Enterprise instance using Harness’s OAuth provider configuration, instead of legacy token-based connectors. It uses the OAuth redirect_uri to obtain access tokens via GitHub Enterprise. **This feature is in Limited GA.** |
