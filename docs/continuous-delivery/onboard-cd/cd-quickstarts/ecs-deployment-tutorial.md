...
4. In Credentials, enter the following and click **Continue**:
	...
	3. **Secret Key:** add a new Harness Secret using the access key's secret key. Secrets are always stored in encrypted form and decrypted only when they are needed. To learn more about Harness secrets, go to [Harness Secrets Management Overview](../../../platform/Secrets/Secrets-Management/1-harness-secret-manager-overview.md).
