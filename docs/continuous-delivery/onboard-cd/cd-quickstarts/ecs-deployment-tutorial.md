...
4. In Credentials, enter the following and click **Continue**:
	...
	3. **Secret Key:** add a new Harness Secret using the access key's secret key. Secrets are always stored in encrypted form and decrypted only when they are needed. To learn more about Harness secrets, go to [Harness Secrets Management Overview](../../../platform/7_Connectors/Secret-Managers/1-harness-secret-manager-overview.md).
...
- Harness File Store.
- AWS S3 buckets.
  - For S3, you use a Harness AWS Connector. The IAM role permissions required by Harness for S3 are described in [AWS Connector Settings Reference](../../../platform/7_Connectors/Cloud-Providers/references/aws-connector-settings-reference.md#aws-s3).
- Git providers.
