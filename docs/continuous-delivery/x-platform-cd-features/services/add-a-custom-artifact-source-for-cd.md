...
* **Script**: Enter a script to fetch a JSON payload and add it to the Harness variable `$HARNESS_ARTIFACT_RESULT_PATH`. 
  ...
  You can use [Harness text secrets](/docs/platform/Security/add-use-text-secrets.md) in the script. For example:
...
[Harness Variables and Expressions](/docs/platform/Variables-and-Expressions/harness-variables.md) can be added to Values files (for example values.yaml), not the manifests themselves. This provides more flexibility.
