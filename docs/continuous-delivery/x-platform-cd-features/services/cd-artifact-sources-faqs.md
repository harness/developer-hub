...
### Why are artifacts successfully uploaded to Artifactory but not appearing in the Harness Artifacts tab?

The Artifacts tab only displays artifacts generated by out-of-the-box (OOTB) Harness steps, not manually uploaded or generated artifacts from custom steps.  Manually uploaded artifacts to Artifactory will not be reflected in the Harness Artifacts tab.

### How can I create a multi-architecture artifact using output from parallel ARM and AMD cloud runners?

Creating a multi-architecture artifact from parallel ARM and AMD cloud runners involves using the `docker manifest create` command to combine the images built on each runner.  The command combines the images into a single manifest that can be pushed to a container registry. Credentials for pushing to ECR can be handled via IAM roles, AWS access keys, or using OIDC, depending on the cloud runner setup.

### How do I bypass artifact validation in a Harness pipeline?
To bypass artifact validation, select the **Skip Validation for the Stage** checkbox in your service settings. This prevents the pipeline from validating artifacts at runtime, which is useful when artifacts are defined dynamically. Note that if any steps in the pipeline execution depend on the artifact, those steps may fail.
