...
We do not have a separate option for inline values yaml override. However we allow to use values override from Harness file store. So we can create the values yaml override in harness file store and add it in the values override configuration.
