...
Create input variables to be used by other fields within the step. The **Value** setting can contain fixed values, expressions or inputs.

These variables can be used by other fields like URL using the expression ```<+spec.inputVariables.variable_name>``` or ```<+step.spec.inputVariables.variable_name>```
