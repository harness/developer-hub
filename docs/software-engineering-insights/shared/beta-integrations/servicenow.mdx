...
import snowCustom from '@site/docs/software-engineering-insights/propelo-sei/setup-sei/configure-integrations/static/snow-custom.png';
import snowField from '@site/docs/software-engineering-insights/propelo-sei/setup-sei/configure-integrations/static/snow-field.png';
...
<iframe
  width="100%"
  height="400"
  src="https://www.youtube.com/embed/wFWHAAIj3_o"
  title="YouTube video"
  frameBorder="0"
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`itil`**                   | `/api/now/table/incident`<br />`/api/now/table/change_request`<br />`/api/now/table/sys_user/<sys_id>`<br />`/api/now/table/sys_user?sysparm_limit=1&sysparm_query=user_name=<USER_NAME>`<br />`/api/now/table/cmdb_ci/<sys_id>`<br />`/api/now/table/cmdb_ci_service/<sys_id>` | Provides access to IT Service Management (ITSM) data such as incidents, change requests, users, and CMDB records.                                                  |
...
- **oauth_admin** (required only for app creation)
...
- **user_admin**: Provides administrative capabilities over user accounts (only required if extended user attributes must be fetched).
...
1. Click **Next: Validate Connection** to establish a connection with the provided URL, perform authentication, and run the preflight checks. If everything is successful, you will see a message confirming a successful connection.
...
1. Click **Next: Validate Connection**. It will establish a connection with the provided URL, perform authentication, and then run the preflight checks. If everything is successful, you will see a message confirming a successful connection.
...
### ServiceNow field mapping and display values
...
<img src={snowCustom} alt="ServiceNow custom field mapping in SEI" width="500" />
...
<img src={snowField} alt="ServiceNow standard field selected from the workflow filter dropdown" width="800" />

</details>

### Recreate ServiceNow queries
