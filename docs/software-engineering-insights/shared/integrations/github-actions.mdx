...
If you're using OAuth, re-authenticating as an [SEI Admin](/docs/software-engineering-insights/harness-sei/get-started/rbac#out-of-the-box-roles) refreshes the access token.

1. In GitHub, go to **Settings** → **Applications** → **Authorized OAuth Apps**.
1. Locate the **SEI app**.
1. Revoke access.
1. Return to Harness SEI.
1. Select your GitHub Actions integration.
1. Click **Re-Authenticate** and complete the OAuth flow.
...
1. In Harness SEI, go to **Integrations**.
1. Select your GitHub Actions integration.
1. Click **Re-Authenticate**.
1. Enter the new token and validate the connection.
...
1. Ensure the token includes required permissions.
1. Update the credentials in your Satellite configuration if applicable.
1. In Harness SEI, go to **Integrations**.
1. Select your GitHub Actions integration.
1. Click **Re-Authenticate**.
1. Re-authenticate the integration using the updated token.
