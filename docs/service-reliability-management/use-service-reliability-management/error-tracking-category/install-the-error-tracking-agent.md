...
| Linux Operating System:* Ubuntu: 14+
* Debian
* CentOS: 6.5+
* RedHat: 5.0 +
* Suse: SLES12
...
### Step: Install an Error Tracking Agent
...
3. Add JVM arguments which instruct the JVM to load the Agent. This is done by adding`-agentpath:/home/user/harness/lib/libETAgent.so`to the application startup. For example,`java -agentpath:/home/user/harness/lib/libETAgent.so -jar yourapp.jar.`. This parameter can also be specified using`JAVA_TOOL_OPTIONS`. For example,`exportJAVA_TOOL_OPTIONS=-agentpath:/home/user/harness/lib/libETAgent.so.`.
