...
End result is that hackers cannot do tampering during the build process. This capability when coupled with open source governance through [SBOM lifecycle management](https://developer.harness.io/docs/supply-chain-security/sbom/generate-sbom) provides the most advanced shift-left supply chain security solution in the market today.
...
To configure attestation, along with the [above configuration](#slsa-generation-step-configuration) you should enable the **SLSA Attestation** checkbox in the **SLSA Generation** step. This requires a key pair generated using **Cosign**. To understand the attestation process please refer to [attestation and verification](/docs/supply-chain-security/get-started/key-concepts#attestation-and-verification) concepts.
...
For verifying the SLSA attestation, please refer to [Verify SLSA](/docs/supply-chain-security/slsa/verify-slsa) documentation.
