...
After you sign the artifact using the [Artifact Signing step](/docs/supply-chain-security/artifact/sign-artifacts), it’s crucial to verify that the artifact has not been tampered and was signed by a trusted source. The Artifact Verification process enables you to validate the integrity and authenticity of the signed artifact before it’s deployed.
...
Once the artifact is signed and verified, you will be able to see the Artifact Integrity Verification status from the [Artifacts Overview](/docs/supply-chain-security/artifact-view#artifact-overview) tab.
