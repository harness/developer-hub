...
description: Learn how to secure your software supply chain using Dependency Firewall with Open Policy Agent policies and policy sets in Harness Artifact Registry.
sidebar_position: 1
keywords:
  - dependency firewall
  - open policy agent
...
- **Dependency Firewall feature flag** (`HAR_DEPENDENCY_FIREWALL`) enabled. Contact [Harness Support](mailto:support@harness.io) to enable it.
- **Upstream proxy registry** configured for package types such as npm or Maven. Go to [Create an upstream proxy](/docs/artifact-registry/manage-registries/upstream-proxy) to set one up.
...
1. **Allowed licenses with operators:** Defines licenses using two matching strategies:
   - **Exact match (`==`):** For licenses that must match exactly (for example, "MIT").
   - **Regex match (`~`):** For license families using patterns (for example, ".*Apache.*" matches "Apache-2.0", "Apache-1.1", etc.).
...
3. Select the policies you want to include in this policy set (for example, "firewall - license").
...
4. Select **Apply**, then select **Finish**.
...
3. You will see a list of evaluated packages with the following information:
   - **Package Name and Version:** The artifact that was evaluated (for example, `requests 2.31.0`).
   - **Upstream Proxy:** The proxy registry name (for example, `pythonproxy`).
...
1. In the **Dependency Firewall** tab, select a package with a `Warning` or `Blocked` status.
2. The **Violation Details** panel displays:
...
- Go to [Dependency Firewall overview](/docs/artifact-registry/dependency-firewall/overview) to understand the full architecture and concepts.
- Go to [Configure policies and policy sets](/docs/artifact-registry/dependency-firewall/configure-policies) to explore additional policy patterns.
- Go to [Open Policy Agent documentation](https://www.openpolicyagent.org/docs/latest/) to explore the Rego policy language in depth.
