...
3. In Harness, download the Harness Kubernetes delegate YAML file. For instructions, go to [Install a Kubernetes delegate](/docs/platform/delegates/install-delegates/install-delegate).
...
7. If you haven't already installed the delegate, [Install the Kubernetes delegate](/docs/platform/delegates/install-delegates/install-delegate) in your EKS cluster and register the delegate with Harness. When you install the delegate in the cluster, the SA you added is used, and the environment variables `AWS_ROLE_ARN` and `AWS_WEB_IDENTITY_TOKEN_FILE` are added automatically by EKS.
