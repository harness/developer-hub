...
For the standard Harness Kubernetes and Native Helm deployments using Helm charts, see [Use a Helm Repository with Kubernetes](/article/hddm3rgf1y-use-a-helm-repository-with-kubernetes) and [Helm Quickstart](/article/2aaevhygep-helm-quickstart).In addition to this method, you can also simply deploy the Helm chart without adding your artifact to Harness. Instead, the *Helm chart is the artifact*. The Helm chart you provide contains the hardcoded link to the artifact.
...
Looking for the API? You can use the Harness GraphQL to run a Helm chart deployment. See [Deploy Helm Charts Using the API](/article/sbvn6uwcq1-deploy-helm-charts-using-api).New to Harness Kubernetes and Native Helm Deployments?Harness includes both [Kubernetes](/article/7in9z2boh6-kubernetes-quickstart) and [Native Helm](/article/583ojfgg49-helm-deployments-overview) deployments, and you can use Helm charts in both. Here's the difference:  
...
* [Native Helm Deployments Overview](/article/583ojfgg49-helm-deployments-overview)
...
The Harness User Group must have the following [Application Permissions](/article/ven0bvulsj-users-and-permissions) for the Service (or All Services):
...
To add the Helm Repository Artifact Server for your chart repo, follow the steps in [Add Helm Repository Artifact Servers](/article/0hrzb1zkog-add-helm-repository-servers).
...
If you add a values.yaml in **Values YAML Override** and you have a values.yaml in your chart already, then Harness will merge them at runtime. The values.yaml in **Values YAML Override** overrides at a `key:value` level, not a file level. If you have a values.yaml in your chart with `key:value` pairs you do not want, you must remove them from that file or override them in the values.yaml in **Values YAML Override**. See .See [Override Values YAML Files](/article/p453sikbqt-override-values-yaml-files) and [Helm Services](/article/svso08ogpb-2-helm-services).
...
Follow the steps in [Define Your Kubernetes Target Infrastructure](/article/u3rp89v80h-define-your-kubernetes-target-infrastructure) or [Helm Environments](/article/134kx1k89d-3-helm-environments).
...
You can also overwrite Service variables at the Phase-level of a multiple Phase Workflow.If you add a values.yaml in **Service Configuration Overrides** and you have a values.yaml in your chart already, then Harness will merge them at runtime. The values.yaml in **Service Configuration Overrides** overrides at a `key:value` level, not a file level. If you have a values.yaml in your chart with `key:value` pairs you do not want, you must remove them from that file or override them in the values.yaml in **Service Configuration Overrides**. See [Variable Override Priority](/article/benvea28uq-variable-override-priority).For details, see [Override Harness Kubernetes Service Settings](/article/ycacqs7tlx-override-harness-kubernetes-service-settings) or [Helm Environments](/article/134kx1k89d-3-helm-environments).
...
3. In **Override Type**, select **Values YAML**. Click **Local** or **Remote**.
	...
	2. **Remote** - See [Override Remote Values YAML Files](https://docs.harness.io/article/p453sikbqt-override-remote-values-yaml-files).
...
For steps on creating a Native Helm Basic Workflow, see [Helm Workflows and Deployments](/article/m8ra49bqd5-4-helm-workflows).
...
You can use these expressions in a [Shell Script](/article/1fjrjbau7x-capture-shell-script-step-output) Workflow step after the deployment step in the Workflow:
...
You can add your Workflow to a stage in a Harness [Pipeline](/article/zc1u96u6uj-pipeline-configuration) or have it executed by a Harness [Trigger](/article/xerirloz9a-add-a-trigger-2).
