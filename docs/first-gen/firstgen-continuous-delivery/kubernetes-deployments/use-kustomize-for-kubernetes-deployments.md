...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582677066267/image.png)The Harness Kubernetes Delegate runs in the target cluster with Kustomize pre-installed. The Delegate obtains kustomization.yaml and resource files from a Git repo. The Delegate deploys the Kubernetes objects declared using Kustomize in the target pods.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582584606916/image.png)1. In **Remote Manifests**, in **Manifest Format**, click **Kustomization Configuration**.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1583175996804/image.png)If you are using overlays, enter the path to the overlay kustomization.yaml.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1642111092925/image.png)When the kustomization.yaml is deployed, the patch is rendered and the environment variable is added to the deployment.yaml that is deployed.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1642111167347/clean-shot-2022-01-13-at-13-59-04.png)You can add multiple files by using **Add Patches** multiple times.In **Store Type**, select **Inline** or **Remote**.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1642111684954/clean-shot-2022-01-13-at-14-07-57.png)One variable is for the `image` and another for the `replicas` count.
...
You can also use [Harness secrets](/article/ygyvp998mu-use-encrypted-text-secrets) in patches.

For example, let's say we have two secrets, one for `image` and one for `app`:

![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1642111957182/clean-shot-2022-01-13-at-14-12-28.png)The following patch uses these secrets for `image` and `app`, referencing them using the expression `${secrets.getValue("[secret name]")}`.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1642112180574/clean-shot-2022-01-13-at-14-15-51.png)In **Store Type**, select **Inline** or **Remote**.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1583176053132/image.png)A better method is to use a single Service for all bases and manually or dynamically identify which base to use at deployment runtime.

You can accomplish this using Harness Variable Expressions in **Path to kustomization directory**

![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1583176070761/image.png)##### Environment Name Variables
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582589475037/image.png)The kustomization.yaml file in the root will reference these folders of course:
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582589395010/image.png)Next, use the built-in Harness variable expression `${env.name}` in **Path to kustomization directory** to use the Environment names. The `${env.name}` expression resolves to the name of the Harness Environment used by a Workflow.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582588881870/image.png)Now, to use the `${env.name}` expression in **Path to kustomization directory**, and reference the Environments and corresponding folders, you would enter `kustomize/multibases/${env.name}`.

![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1583176200352/image.png)Each time a Workflow runs, it will replace the `${env.name}` expression with the name of the Environment selected for the Workflow.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582663979736/image.png)You can also select the Environment in a Trigger than executes the Workflow:

![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582664466503/image.png)For more information, see [Triggers](/article/xerirloz9a-add-a-trigger-2) and [Passing Variables into Workflows and Pipelines from Triggers](/article/revc37vl0f-passing-variable-into-workflows).
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1583176347452/image.png)If you have Service **Config Variables** set up, you will see the variable expressions displayed when you enter `$`. For details on Service variables, see [Services](/article/eb3kfl8uls-service-configuration).
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1583176437929/image.png)If you use Workflow variables for **Path to kustomization directory**, you can provide a value for **Path to kustomization directory** when you deploy the Workflow (standalone or as part of a Pipeline).

![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582586424096/image.png)Typically, when you deploy a Workflow, you are prompted to select an artifact for deployment. If a Workflow is deploying a Service that uses a remote **Kustomization Configuration**, you are not prompted to provide an artifact for deployment.See [Workflows](/article/m220i1tnia-workflow-configuration) and [Kubernetes Workflow Variable Expressions](/article/9dvxcegm90-variables).
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582665881519/image.png)1. Click **Confirm**.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1583176526837/image.png)Click **Submit**. Harness is now configured to use the plugin when it deploys using kustomize.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582672315326/image.png)Here are the Harness Environments whose names correspond to the dev, stage, and production repo folders:

![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582672712119/image.png)Here is the Harness Service **Remote Manifests** settings. The **Path to kustomization directory** setting uses the `${env.name}` expression that will be replaced with a Harness Environment name at deployment runtime.

![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1583176601635/image.png)Next we'll create a Workflow using the Rolling Deployment strategy. Here we select the Service we set up.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582673459760/image.png)There is nothing to set up in the Workflow. Harness automatically adds the Rollout Deployment step that performs the Kubernetes Rolling Update.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582678843311/image.png)In this example, we select the **stage** Environment. Once deployment is complete you can see the stage repo folder's base used and the `staging-myapp-pod` created:

![](https://files.helpdocs.io/kw8ldg1itf/articles/zrz7nstjha/1582674470111/image.png)### Review: What Workloads Can I Deploy?
