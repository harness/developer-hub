...
This content is for Harness [FirstGen](https://docs.harness.io/article/1fjmm4by22). Switch to [NextGen](/article/mlpksc7s6c-single-sign-on-saml).Harness supports Single Sign-On (SSO) with SAML, integrating with your SAML SSO provider to enable you to log your users into Harness as part of your SSO infrastructure.

If the [Harness On-Prem](/article/gng086569h-harness-on-premise-versions) version is not accessed using HTTPS load balancer, SAML authentication will fail. Make sure you access the Harness On-Prem version using a HTTPS load balancer, and not a HTTP load balancer.This document explains how to set up SAML authentication and authorization.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1578095422568/image.png)![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1598475007686/image.png)### SAML SSO with Okta Overview
...
2. Using your Harness Users email addresses, add the Harness Users to the SAML provider account and assign them to your SAML provider app. Here is an example of two Harness Users added to an Okta app using the same email addresses.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1578095470391/image.png)
...
For details about the Harness side of the information exchanges described throughout this topic, see [Authentication Settings](/article/zlymcu6bf1-authentication-settings).The first step is to add the Harness app to your Okta account.

1. In your Okta administrator account, click **Applications**.
2. In **Add Application**, search for **Harness** and select it.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1598474561454/image.png)
3. On the Harness app page, click **Add**.
4. In **General Settings**, click **Done**.

The Harness app is for the Harness SaaS edition. For Harness On-Prem editions, contact [Harness Support](mailto:support@harness.io).##### Add the Harness SAML Endpoint URL to Harness Okta App

1. In the Harness Okta app, click the **Sign On** tab, and then click **Edit**.
2. In **ACS URL** field, paste the Harness SAML Endpoint URL you copied.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1598474850795/image.png)
...
For Harness [On-Prem](/article/gng086569h-harness-on-premise-versions), replace **app.harness.io** with your custom URL.  
If you use a custom Harness subdomain in any Harness version, like **example.harness.io**, use that URL.#### Okta SAML Metadata File
...
For example, here is what the SAML metadata file section looks like in an Okta application's **Sign On** tab (only available in Classic UI view):![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561408714574/image.png)
2. In Harness' **Add SAML Provider** dialog, under **Upload a new SAML Metadata File**, click **Choose File** and add the SAML metadata file you downloaded.![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561184691757/image.png)
3. In **Display Name**, enter a name for your SAML SSO provider, and click **SUBMIT**. The new SSO provider is displayed.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1564538959861/image.png)

#### Enable and Test SSO with Okta

Now that Okta is set up in Harness as a SAML SSO provider, you can enable and test it.

1. To enable the SSO provider, drag the slider under **Authentication** **Enabled?**![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1564539138747/image.png)
2. In the resulting **Enable SSO Provider** dialog, click **TEST** to verify the SAML connection you've configured.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1578095649978/image.png)A new browser tab will prompt you to log into your SAML provider.

If you do not see a new tab, your browser might have blocked it with a pop-up blocker. Please disable the blocker temporarily and retry the TEST button.1. Upon a successful test, Harness will display the **SAML test successful** banner below.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1578095679227/image.png)When you see this banner, it is safe to click **CONFIRM** to enable SAML SSO in Harness.  
...
In Okta, here is a user group named **QA** with two members:![](https://files.helpdocs.io/kw8ldg1itf/articles/39wgyobusz/1552504533997/image.png)Both members are already registered in Harness using the same email addresses in both Harness and the SAML provider.![](https://files.helpdocs.io/kw8ldg1itf/articles/39wgyobusz/1552504745805/image.png)
3. Ensure that the SAML provider group is assigned to the same SAML provider app you use for Harness SAML SSO. Here is the same SAML provider group assigned to an app in Okta:![](https://files.helpdocs.io/kw8ldg1itf/articles/39wgyobusz/1552504885698/image.png)
...
Here are the steps for adding a Group Attribute Name in Okta:
	...
	6. In **Filter**, select **Matches regex** and enter period asterisk (**.\***) in the field. When you are done, it will something like this:![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1552517428145/image.png)
	7. Click **Next**.
	8. Click **Finish**.
5. Enable Authorization in Harness. Now that you have assigned a group to your SAML app and added a Group Attribute Name, you can enable authorization in Harness.
	...
	(Remember, we are simply adding authorization to a SAML SSO provider that is already configured for authentication and **enabled**, as described in [To Set Up SAML SSO](#to_set_up_saml_sso).)![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561355330201/image.png)
	4. In the **Edit SAML Provider** dialog, enable **Enable Authorization**.
	5. In **Group Attribute Name**, enter the Group Attribute Name you earlier added to your SAML app in your SAML provider.![](https://files.helpdocs.io/kw8ldg1itf/articles/39wgyobusz/1552509844657/image.png)
...
8. Link the SAML SSO Provider to the Harness User Group. We will create a new Harness User Group, but you can use an existing Group so long as your Harness User account is a member and that User account is registered using the same email address you used to register with your SAML provider.
	...
	7. Click **SUBMIT**.
	8. In the **User Group** page, in **Member Users**, click **Link to SSO Provider Group**.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1552517560380/image.png)
	9. In the **Link to SSO Provider Group** dialog, in **SSO Provider**, select the SAML SSO Provider you have set up.
	10. In **Group Name**, enter the name of the group you assigned to your app in your SAML provider. When you are done, it will look something like this:![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1552517648325/image.png)
	11. Click **SUBMIT**.
...
Do not log out of Harness. Use a different Harness User account in a Chrome Incognito window to ensure that you can disable SSO if needed.
	...
	When the user submits their email address in Harness Manager, the user is redirected to the SAML provider to log in. For example, here is the Okta prompt:![](https://files.helpdocs.io/kw8ldg1itf/articles/39wgyobusz/1552515326619/image.png)
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1552518007487/image.png)The **Delink External SSO Provider Group** dialog appears.

![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1552518040710/image.png)Click **Retain all members in the user group** and click **Confirm**. The link to the SAML SSO Provider is removed and the Users are still members of the User Group.
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561412201466/image.png)#### Azure User Accounts
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553548722461/image.png)#### Endpoint URL for Azure

You must enter the **Harness SAML Endpoint URL** from Harness in your Azure app **Reply URL**:

1. In your Azure app, click **Single sign-on**. The SSO settings for the Azure app are displayed.![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553552828203/image.png)
2. In **Basic SAML Configuration**, click the edit icon (pencil).
3. Enter **app.harness.io** in the **Identifier (Entity ID)** field.![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553552929086/image.png)Next, you will use the **SAML SSO Provider** settings in Harness to set up your Azure app **Single sign-on**.
...
2. Click **SSO Providers**, and then click **Add SSO Provider** and choose **SAML**. The **Add SAML Provider** dialog appears.![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561412418775/image.png)
3. In **Display Name**, enter a name for the SAML SSO Provider, such as **Azure**.
4. Copy the **Harness SAML Endpoint URL** from the **Add SAML Provider** dialog, and paste it in the **Reply URL** in your Azure app.![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561418057379/image.png)
5. Click **Save**. The Basic SAML Configuration now looks like this:![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553553199711/image.png)
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1553620383775/image.png)To set this **User name** email address as the method for identifying users, in the Azure app **Single sign-on** section, the Azure app must use the **user.userprincipalname** as the **Unique User Identifier**, and **user.userprincipalname** must use **Email address** as the **name identifier format**.

To set this up in your Azure app, do the following:

1. In your Azure app, in the **Single sign-on** blade, in **User Attributes & Claims**, click the edit icon (pencil). The **User Attributes & Claims** settings appear.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1553618885450/image.png)
2. For **Name identifier value**, click the edit icon. The **Manage user claims** settings appear.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1553619139106/image.png)
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1553618593604/image.png)If your Azure users are set up with their email addresses in some field other than **User name**, just ensure that the field is mapped to the **Unique User Identifier** in the Azure app and the **name identifier format** is **Email address**.#### Azure SAML Metadata File

You must download the **Federation Metadata XML** from your Azure app and upload the file into Harness.

1. Download the **Federation Metadata XML** from your Azure app and upload it using **Upload a new SAML Metadata File** in the **Add SAML Provider** dialog in Harness.![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561418183283/image.png)
2. In Harness' **Add SAML Provider** dialog, click **SUBMIT**. The new Azure SAML Provider is added.![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561358314876/image.png)
...
1. In the Azure app, click **Single sign-on**, and then at the bottom of the **Single sign-on** settings, click **Test**.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1553623078852/image.png)
...
4. Enter the Azure user name for the user (most often, the email address), enter the Azure password, and click **Sign in**.![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553551644635/image.png)
...
![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561418679595/image.png)For Harness **Group Name**, here is the Harness User Group link setting on the left and the Azure group Object ID on the right.

![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553627861768/image.png)To set up Azure Authorization in Harness, do the following:

1. In Azure, add the **Group Claim** (Name and Namespace) to the Azure app.
	1. In your Azure app, click **Single sign-on**, and then in **User Attributes & Claims**, click edit (pencil icon).![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1553633401768/image.png)
	2. In **User Attributes & Claims**, edit the **Groups returned in claim**. The **Group Claims** settings appear.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1553633448139/image.png)
	...
	5. In **Namespace**, enter a namespace name. When you are done, **Group Claims** will look something like this:![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553631370519/image.png)
	6. Click **Save**. **User Attributes & Groups** now displays the group claim you created.![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553631562312/image.png)
...
2. In Harness, enter the Group Claim name and namespace in the SAML SSO Provider **Group Attribute Name** field.
	...
	2. Enter the Group Claim name and namespace in the **Group Attribute Name** field in the same format as a Claim Name (`namespace/name`). The SAML SSO Provider dialog will look something like this:![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553631848104/image.png)
...
5. In Azure, ensure the Azure users with corresponding Harness accounts belong to an Azure group. Here is an Azure group named **Example** with two members:![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553631987585/image.png)
6. Ensure that the Azure group is assigned to the Azure app. Here you can see the **Example** group in the Azure app's **Users and groups**:![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553632064222/image.png)
7. Link the Harness User Group to the Azure group using the Azure group Object ID.
	1. In Azure, copy the Azure group **Object ID**.![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553632193833/image.png)
	2. In Harness, create a new User Group or open an existing User Group.
	3. In the **Member Users** section, click **Link to SSO Provider Group**.![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553632304646/image.png)
	4. In the **Link to SSO Provider Group** dialog, in **SSO Provider**, select the Azure SSO Provider you set up, and in **Group Name**, paste the Object ID you copied from Azure. When you are done, the dialog will look something like this:![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553632522628/image.png)
	5. Click **SUBMIT**. The User Group is now linked to the SAML SSO Provider and Azure group Object ID.![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553632592994/image.png)
8. Test Authorization.
	...
	3. In the linked Harness User Group, ensure that the Harness User account you logged in with was added.![](https://files.helpdocs.io/kw8ldg1itf/articles/woihlvoeac/1553632825160/image.png)The Harness User is now added and the RBAC settings for the Harness User Group are applied to its account. For more information, see [Managing Users and Groups (RBAC)](/article/ven0bvulsj-users-and-permissions).
...
Users are not created as part of the SAML SSO integration. Users are invited to Harness using their email addresses. Once they log into Harness, their email addresses are registered as Harness Users. For more information, see [SAML SSO with Harness Overview](#saml_sso_with_harness_overview).#### Users in over 150 Groups
...
6. Make sure to copy this secret and save it as an encrypted text secret. For detailed steps to create an encrypted text in Harness, see [Use Encrypted text Secrets](/article/ygyvp998mu-use-encrypted-text-secrets).
7. Select the above secret reference in the Client Secret field in your Harness account.
8. Select **Add Entity ID** and enter your custom Entity ID. The default Entity ID is **app.harness.io**. The value you enter here will override the default Entity ID.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1643304453368/screenshot-2022-01-27-at-10-53-44-pm.png)
...
To set up SAML with Google, you must be a G Suite administrator. To integrate G Suite SAML with Harness, your Harness account must be a member of the Account Administrator group. For more information, see [Users and Permissions](/article/ven0bvulsj-users-and-permissions).First, we will set up Harness as a SSO SAML app with Google G Suite, and then we will configure Harness to use Google G Suite for SSO.

If you become locked out of Harness because of a SSO issue, you must contact Harness to have the SSO Provider disabled.#### Add Harness as G Suite SSO App

To set up SAML with Google G Suite, do the following:

1. Log into your G Suite Admin console.
2. From the Admin console Home page, go to **SAML apps**.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1536866084231/image.png)
3. Click the plus sign next to **Enable SSO for a SAML Application**.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1536866136460/image.png)
4. In **Step 1**, at the bottom, click **SETUP MY OWN CUSTOM APP**.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1536866200030/image.png)
5. In **Step 2**, the **Google IdP Information** appears, including the **SSO URL** and **Entity ID** URLs.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1536866326018/image.png)
	...
	2. Click **NEXT**.
6. In **Step 3**, in **Application Name**, enter a name for the application. Make sure that the name does not include spaces or special characters.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1599716435830/new-gsuite-samlsso.png)
...
7. In **Step 4**, provide the Harness provider details.
	...
	2. On the resulting **Authentication Settings** page, click **Add SSO Provider**, and click **SAML**.![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561359076776/image.png)The **Single Sign-on (SSO) Provider** dialog appears.
	3. Copy the **Harness SAML Endpoint URL** from the dialog and enter it in **ACS URL** in G Suite.![](https://files.helpdocs.io/kw8ldg1itf/other/1571178530723/image.png)
	4. In **Entity ID**, enter **app.harness.io**.For Harness [On-Prem](/article/gng086569h-harness-on-premise-versions), replace **app.harness.io** with your custom URL.  
...
9. In the Harness service provider page, click the **pencil icon** to turn on the new Harness SSO app.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1536867096145/image.png)
10. In the **Status** setting, click **Settings for all organizational units**.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1536867231065/image.png)Make sure you select this option. You'll get an error saying that the service is not enabled for the user while testing the SAML connection if you don't select this option.
11. In the **Service Status** settings, click **ON for everyone**, and then click **SAVE**.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1536867286403/image.png)
...
3. Click **Add SSO Provider**. The **Single Sign-on (SSO) Provider** dialog appears.![](https://files.helpdocs.io/kw8ldg1itf/articles/i4ftgq1vel/1561412634127/image.png)
...
6. Back in the **Single Sign-on(SSO) Provider Setup** list, enable the new SSO provider.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1536867755595/image.png)
7. You are finished. To test the provider, log out of Harness.
8. Open a G Suite app, such as **Mail**, and locate the Harness app you added by clicking the Google Apps icon.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1536867893316/image.png)
9. Click the **Harness** app icon and you will be redirected to **app.Harness.io**, and logged in.

For Harness [On-Prem](/article/gng086569h-harness-on-premise-versions), you will be directed to your custom URL.  
If you use a custom Harness subdomain in any Harness version, like **example.harness.io**, it will be that URL.### SAML SSO with OneLogin
...
Users are not created as part of the SAML SSO integration. Users are invited to Harness using their email addresses. Once they log into Harness, their email addresses are registered as Harness Users. For more information, see [SAML SSO with Harness Overview](#saml_sso_with_harness_overview).#### OneLogin Authentication on Harness
...
2. In the resulting **Add SAML Provider** dialog, copy the provided URL to the clipboard.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583909424913/image.png)
3. In OneLogin, add the **Harness** app (for SaaS setup) or **Harness (On Prem)** app for On-Prem setup. To do so, perform the following steps:
	...
	4. Click **Add App**.
	5. Find **Harness** or **Harness (On Prem)** based on your setup, and then click the app.![](https://files.helpdocs.io/kw8ldg1itf/articles/zy8yjcrqzg/1617186215490/screenshot-2021-03-31-at-3-51-27-pm.png)
4. In **Configuration**, paste this URL into the **SCIM Base URL** field.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583910413585/image.png)
5. Skip all other **Application Details** fields, and click **Save**.
6. Navigate to OneLogin's **Applications** > **SSO** tab. At the upper right, select **More Actions** > **SSO Metadata**.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583398317192/image.png)
...
2. Search for a user that you want to add to Harness.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583869787224/image.png)
3. Click to select the user.
4. On the resulting **Users** page, select the **Applications** tab at left.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583903157026/image.png)
5. Click the **+** button at upper right to assign an Application.
6. Find and select the Application, then click **Continue**.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583903896797/image.png)
...
6. Select **Add Entity ID** and enter your custom Entity ID. The default Entity ID is **app.harness.io**. The value you enter here will override the default Entity ID.
7. Select **Submit**.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583909918124/image.png)This adds OneLogin as a SSO Provider, and returns you to Harness **Authentication Settings**,
8. Slide the **Authentication enabled?** slider right, to enable your new provider.
9. In the resulting **Enable SSO Provider** dialog, click **Test**.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583398196969/image.png)
...
1. From OneLogin's, menu, select **Users** > **Users**.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583910749488/image.png)
2. Click to select a user, assigned to Harness, whom we want to assign to appropriate OneLogin Roles.
3. Select the **Applications** tab at left.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583911049171/image.png)
4. As shown above, click each Role that you want to assign to this user.
5. Click **Save User** at upper right.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583911437230/image.png)
...
4. In **Group Attribute Name**, type paste in the name of your the [Field Group](#group_parameters) you configured in OneLogin.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583481125420/image.png)
...
10. Click **SUBMIT.**![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583481487829/image.png)The link now appears on the User Group's **Member Users** record.![](https://files.helpdocs.io/kw8ldg1itf/articles/xhx9et5f44/1583481564433/image.png)
...
You cannot delete a SAML SSO Provider from Harness that is linked to a Harness Group. You must first remove the link to the SSO Provider from the Group.### SAML SSO FAQs
...
* To migrate from Okta SAML to Okta SCIM integration with Harness, see [Migrating to Okta SCIM from Okta SAML](/article/8hwnfif0v1-scim-okta#migrating_to_okta_scim_from_okta_saml).
