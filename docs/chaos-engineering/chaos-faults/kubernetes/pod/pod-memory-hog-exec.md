  ...
  <h3>Optional fields</h3>
    <table>
      ...
      <tr>
        ...
        <td> Defaults to 500MB (Up to 2000MB). For more information, go to <a href="https://developer.harness.io/docs/chaos-engineering/chaos-faults/kubernetes/pod/pod-memory-hog-exec#memory-consumption"> memory consumption. </a></td>
      </tr>
      <tr>
...
It specifies the amount of memory consumed by the target pod. The memory consumption limit is 2000 MB. Tune it by using the `MEMORY_CONSUMPTION` environment variable.
...
It specifies the kill command that is set to exhaust the resources. Tune it by using the `CHAOS_KILL_COMMAND` environment variable.

- `CHAOS_KILL_COMMAND`: "kill $(find /proc -name exe -lname '\*/dd' 2>&1 | grep -v 'Permission denied' | awk -F/ '{print $(NF-1)}' | head -n 1)"

Use the following example to tune the kill command:
