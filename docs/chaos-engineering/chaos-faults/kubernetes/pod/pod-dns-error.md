...
It specifies the comma-separated names of the target hosts that are subject to chaos. If `TARGET_HOSTNAMES` is not provided, all host names or domains will be targeted. Tune it by using the `TARGET_HOSTNAMES` environment variable.

Use the following example to tune the target host names:
...
It specifies whether the DNS query has to exactly match with one of the targets or can have any of the targets as a substring. It supports `exact` or `substring` values. Tune it by using the `MATCH_SCHEME` environment variable. 
...
Use the following example to tune container runtime and socket path:
