...
Linux chaos infrastructure daemon service is a **Systemd** service that is responsible for injecting faults into a Linux machine as part of a chaos experiment.
...
- Multiple faults in an experiment can execute in serial or parallel to each other, as per the definition.
...
- Chaos rollback is performed at the end of each experiment execution. Also, on-the-fly experiments can be safely aborted and the chaos is reverted.
...
- In the rare scenario where the revert of chaos itself also leads to an error, an appropriate error message is logged in the experiment log for the manual intervention required by the user.
