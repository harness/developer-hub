...
- It also helps validate the [declarative hypothesis](#declarative-hypothesis) set by the user.
...
Declarative hypotheses in HCE is written in a simple, clear and concise manner, that is, it should be **specific, measurable, achievable, relevant and time-bound (SMART)**. The steady state and declarative hypothesis set by the user should directly map with the SLOs.
...
- Probes are **resuable**, that is, you can create the validation instance first and use it across different experiments.
