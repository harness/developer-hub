...
sidebar_label: Pod JVM SQL Exception
description: Cause JDBC calls from a JVM running in a target Kubernetes pod to throw a configurable exception on a chosen table and SQL operation so you can test caller error handling.
keywords:
  - chaos engineering
...
:::tip Use a realistic exception class
...
- **Application error rate:** Use an [HTTP probe](/docs/resilience-testing/chaos-testing/probes/http-probe) against endpoints that read or write to the targeted table.
- **JDBC error metrics:** Use a [Prometheus probe](/docs/resilience-testing/chaos-testing/probes/apm-probes) on `jdbc.connections.errors` or your APM's database error counter.
...
<Troubleshoot
  issue="Pod JVM SQL exception experiment stays Pending or never starts in Harness Chaos Engineering"
  mode="docs"
...
<Troubleshoot
  issue="No exception observed during pod-jvm-sql-exception"
  mode="docs"
...
<Troubleshoot
  issue="Connection to container runtime fails for pod-jvm-sql-exception in Harness Chaos Engineering"
  mode="docs"
...
- [Pod JVM SQL latency](/docs/chaos-engineering/faults/chaos-faults/kubernetes/pod/pod-jvm-sql-latency): Add latency to SQL statements instead of failing them.
- [Pod JVM Mongo exception](/docs/chaos-engineering/faults/chaos-faults/kubernetes/pod/pod-jvm-mongo-exception): MongoDB equivalent for the MongoDB Java driver.
