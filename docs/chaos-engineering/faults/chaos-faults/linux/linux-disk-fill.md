...
sidebar_label: Linux Disk Fill
description: Fill a disk path on a target Linux machine to a configured size for a configurable duration so you can test how the workload behaves when storage runs out.
keywords:
  - chaos engineering
  - linux disk fill
  - linux fault
...
<Troubleshoot
  issue="Linux disk fill fault fails with no such file or directory in Harness Chaos Engineering"
  mode="docs"
  fallback="FILL_PATH must exist on the target VM and be writable by the linux-chaos-infrastructure service (which runs as root). Verify with sudo test -w FILL_PATH and create the directory if missing with sudo mkdir -p FILL_PATH."
/>
