...
sidebar_label: Azure Instance CPU Hog
description: Drive CPU utilization to a configurable target on one or more Azure VMs for a configurable duration so you can test how the workload behaves when compute headroom shrinks.
keywords:
  - chaos engineering
...
- /docs/chaos-engineering/chaos-faults/azure/azure-instance-cpu-hog
---
...
| Standalone Windows VMs | Supported (use [Windows CPU stress](/docs/chaos-engineering/faults/chaos-faults/windows/windows-cpu-stress) for native Windows support) |
...
- **CPU on the VM:** Use a [Prometheus probe](/docs/resilience-testing/chaos-testing/probes/apm-probes) on `node_cpu_seconds_total` (Linux) or `windows_cpu_time_total` (Windows) and assert the spike is observed.
- **Application latency:** Use an [HTTP probe](/docs/resilience-testing/chaos-testing/probes/http-probe) on the user-visible endpoint and assert p95 stays inside the SLA.
...
<Troubleshoot
  issue="Azure instance CPU hog fails with run-command failed in Harness Chaos Engineering"
  mode="docs"
  fallback="The Azure VM Agent must be running and the run-command extension must be installed. Verify with az vm get-instance-view -g <rg> -n <vm> --query 'instanceView.vmAgent.statuses'. Reinstall the VM Agent if the status is not Ready."
/>

<Troubleshoot
  issue="Azure instance CPU hog fails with AuthorizationFailed"
  mode="docs"
  fallback="The Azure principal is missing Microsoft.Compute/virtualMachines/runCommand/action. Assign Virtual Machine Contributor (or a custom role with the runCommand action) on the target resource group or subscription."
/>

<Troubleshoot
  issue="CPU spike not visible in Azure Monitor"
  mode="docs"
  fallback="Azure Monitor metrics aggregate at 1-minute granularity. For short chaos windows, inspect node-level metrics through the OS (top/Windows Task Manager) or a Prometheus node exporter scrape."
/>
...
- [Azure instance memory hog](/docs/chaos-engineering/faults/chaos-faults/azure/azure-instance-memory-hog): Stress memory instead of CPU.
- [Azure instance IO stress](/docs/chaos-engineering/faults/chaos-faults/azure/azure-instance-io-stress): Stress disk IO instead of CPU.
- [Azure instance stop](/docs/chaos-engineering/faults/chaos-faults/azure/azure-instance-stop): Stop the VM instead of stressing it.
