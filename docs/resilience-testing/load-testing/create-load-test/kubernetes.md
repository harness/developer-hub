...
sidebar_position: 2
description: Create and run a load test on Kubernetes infrastructure in Harness Resilience Testing
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

:::info Kubernetes Support
Load testing on Kubernetes infrastructure is supported from **version 1.85.3** onwards. Load testing is enabled by default on Kubernetes infrastructure — no additional configuration is required.
:::

## Prerequisites

- Access to the Harness Resilience Testing module
- A [Kubernetes Chaos Infrastructure](/docs/resilience-testing/chaos-testing/infrastructure/types/legacy-infra/kubernetes) (v1.85.3 or later)
...
| **Target Type** | Select **Kubernetes** to run the load test on a Kubernetes cluster |
...
The Harness chaos agent in the cluster orchestrates a master pod and (optional) worker pods for load generation.

- Best for scalable, distributed load generation and cloud-native environments
- Supports horizontal scaling with worker pods for higher concurrency
- See [Kubernetes Infrastructure](/docs/resilience-testing/chaos-testing/infrastructure/types/legacy-infra/kubernetes) for setup instructions
...
<TabItem value="image" label="Using Image">

Use a prebuilt container image as the load test source. This is useful when you have a custom Locust setup packaged as a Docker image.

| Field | Description |
|---|---|
| **Host URL** | Base URL of the application under test |
| **Load Test Image** | Container image reference (e.g., `my-registry/my-load-test:latest`) |
| **Entrypoint** | Path to the Locust script inside the container (e.g., `/locust/locust.py`) |
