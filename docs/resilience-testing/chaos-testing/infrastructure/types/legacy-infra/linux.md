...
:::info note
- LCI currently does not support executing parallel faults (see Experiments documentation) in SaaS (the self-managed platform (SMP) supports executing parallel faults on LCI).
:::
...
You can also specify different values (see Key Concepts documentation) for the infrastructure.
...
Go to [Upgrade Saas or SMP for Linux](../../upgrade-infra#upgrade-linux-infrastructure) to upgrade your installation.
