...
With ChaosGuard, each experiment run consists of a security step wherein one or more [rules](#rule) are evaluated before execution. Each rule contains one or more [conditions](#condition) describing the constraints specified in the table above. The experiment can proceed only upon a successful evaluation of all the rules.
...
- [Configure a Condition](./govern-run#configure-a-condition)
- [Configure a Rule](./govern-run#configure-a-rule)