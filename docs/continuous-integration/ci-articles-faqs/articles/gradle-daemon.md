...
To resolve this, add `/tmp` to the stage's [shared paths](/docs/continuous-integration/use-ci/set-up-build-infrastructure/ci-stage-settings#shared-paths). This makes the `/tmp/` directory available across all containers in the stage, allowing the Background step daemon to access the Test Intelligence init script generated by the Test step.
