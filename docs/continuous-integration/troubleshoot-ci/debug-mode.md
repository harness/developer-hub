...
* The build fails at a [Run step](../use-ci/run-ci-scripts/run-step-settings.md) with a Bash or Shell script in a **Build** (`CI`) stage.
* The build runs on a Linux-based OS on Harness Cloud, a self-hosted VM, or a Kubernetes cluster [build infrastructure](../use-ci/set-up-build-infrastructure/which-build-infrastructure-is-right-for-me.md).
