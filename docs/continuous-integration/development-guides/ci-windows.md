...
You can [view test results](/docs/continuous-integration/use-ci/run-tests/viewing-tests) on the **Tests** tab of your pipeline executions. Harness supports JUnit XML and TRX test report formats. TRX reports are automatically converted to JUnit XML.
...
For details about building and testing .NET with Harness CI, including how to setup different versions of the .NET SDK, go to the [C# (.NET Core) guide](./ci-dotnet.md).
