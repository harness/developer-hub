...
### Build X Builds
Customers can use BuildX for their Docker builds. By default, Harness uses Kaniko on Kubernetes. To switch to BuildX by default, please [contact support](https://support.harness.io) to enable the feature flag `CI_USE_BUILDX_ON_K8`. Note that BuildX requires privileged access. Regardless of this flag, BuildX is always used by default when Docker Layer Caching (DLC) is enabled in **Build and Push** steps.
