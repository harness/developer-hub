...
3. Configure the Step as follows.
   ...
   * Under Optional Configuration, add the following settings:
	  ...
	  + `password` = The ID of the text secret that contains the Git provider password, using the convention described [here](/docs/platform/Security/add-use-text-secrets.md#step-3-reference-the-encrypted-text-by-identifier). For example: `<+secrets.getValue("mygithubpersonalaccesstoken")>`
...
The following snippets illustrate the different ways that Drone and Harness CI handle text secrets. See [Add and Reference Text Secrets](/docs/platform/Security/add-use-text-secrets.md).
