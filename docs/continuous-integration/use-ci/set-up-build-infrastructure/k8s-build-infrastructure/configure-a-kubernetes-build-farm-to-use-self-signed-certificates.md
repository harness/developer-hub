...
1. Create a Kubernetes secret or config map with the required certificates in the same namespace used by the Harness delegate. For example:
   ...
   type: Opaque  
   stringData:                             
     ca.bundle: XXXXXXXXXXXXXXXXXXXXXXXXXXX   
       -----BEGIN CERTIFICATE-----  
...
2. Mount the secret as a volume on the delegate pod.
   ...
   Provide a comma-separated list of paths in the build pod where you want the certs to be mounted, and mount your certificate files to `opt/harness-delegate/ca-bundle`.

   <!-- https://harness.atlassian.net/browse/STO-7123?focusedCommentId=672164 -->

   ```yaml
          env:
          volumeMounts:
            - name: certvol
              mountPath: "/etc/ssl/certs/ca-bundle.crt,/kaniko/ssl/certs/additional-ca-cert-bundle.crt"
              subPath:  ca.bundle
          volumes:
           - name: certvol
             secret:
...
2. If you're storing your certificates in a local registry and [need to run Docker-in-Docker](/docs/security-testing-orchestration/sto-techref-category/security-step-settings-reference#docker-in-docker-requirements-for-sto), specify the local certificate path on the delegate.
   ...
   You would then need to configure or extend the `mountPath` on the delegate as follows. Note the `value` path:

   ```yaml
          env:
          volumeMounts:
            - name: certvol
              mountPath: "/etc/ssl/certs/ca-bundle.crt,/kaniko/ssl/certs/additional-ca-cert-bundle.crt,/etc/docker/certs.d/my-registry.local.org:799/ca.crt"
              subPath:  ca.bundle
          volumes:
           - name: certvol
             secret:
