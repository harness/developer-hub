...
If your bucket requires KMS encryption, set the value to `aws:kms` instead:
...
To prevent this issue, Harness can add separators (`/`) to your S3 cache keys to prevent accidental prefix matching and pulling incorrect caches from cloned pipelines. To enable the separator, add this [stage variable](/docs/platform/pipelines/add-a-stage/#stage-variables): `PLUGIN_ENABLE_SEPARATOR: true`.
