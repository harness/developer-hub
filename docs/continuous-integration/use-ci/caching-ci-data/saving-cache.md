...
To prevent this issue, Harness adds separators (`/`) to your AWS cache keys to prevent accidental prefix matching and pulling incorrect caches from cloned pipelines. This feature is enabled by default.

If you want to *disable* the separator, add this [stage variable](/docs/platform/pipelines/add-a-stage/#stage-variables): `PLUGIN_DISABLE_SEPARATOR: true`. If you disable the separator, make sure your pipelines [generate unique cache keys](#key) to avoid the prefix matching issue.
