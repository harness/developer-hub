...
* For kubernetes and VM build infrastructures, use `/addon/bin/split_tests`.
* For steps running locally or in containerized executions on a VM, rather than on the host, use `/usr/bin/split_tests`. You must also declare this path in [shared paths](/docs/continuous-integration/use-ci/set-up-build-infrastructure/ci-stage-settings.md#shared-paths).
