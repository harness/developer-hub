...
`CI` and `DRONE` are equivalent. They identify the build environment as CI. They are always `true` for all builds.
...
The build number. This value is immutable. Equivalent to the expression `<+pipeline.sequenceId>`.
...
Reports `amd64` or `arm64` for VM build infrastructures only.
...
The name of the build infrastructure machine.
...
The build infrastructure's operating system, such as `linux`.
...
The stage build infrastructure type, such as `KUBERNETES_DIRECT` or `docker`.
...
Provides the hostname used by the build infrastructure, such as `localhost`. This can be combined with [`DRONE_SYSTEM_PROTO`](#drone_system_proto) to construct the server URL.

## DRONE_SYSTEM_HOSTNAME

Duplicate of [`DRONE_SYSTEM_HOST`](#drone_system_host).

## DRONE_SYSTEM_PROTO

The protocol used by the build infrastructure, such as `https`. This can be combined with [`DRONE_SYSTEM_HOST`](#drone_system_host) to construct the server URL.
...
Always `/harness`, which is the default [workspace](../caching-ci-data/share-ci-data-across-steps-and-stages.md#share-data-between-steps-in-a-stage).

This is where Harness clones the codebase and it is the working directory for all steps in a stage.
...
* `HARNESS_USER_ID`: ID of the user that started the build.
* `JDK`
* `JFROG_PASSWORD`
* `JFROG_USERNAME`
