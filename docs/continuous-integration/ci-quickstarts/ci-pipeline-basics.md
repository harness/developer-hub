...
You can use shared paths (`sharedPaths`) in a stage to [share data across steps](../use-ci/caching-ci-data/share-ci-data-across-steps-and-stages.md). By default, all steps in a stage use the same workspace to share data. The default shared working directory for a stage is `/harness`. For example, the Maven `m2` repo is stored in `/root/.m2` by default, and you can specify this same path when using the Maven install in a later stage in the same pipeline.
...
In a CI pipeline, you can [run a variety of tests](/docs/category/run-tests), such as integration tests, functional tests, and unit tests.
