...
* **Exit Codes:** In the event that an exit code is defined and set in the script, the output variables will not be available as an output from the step.  This includes `exit 0` definitions.  The output from the step will be empty.
:::
