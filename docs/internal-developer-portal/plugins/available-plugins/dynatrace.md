...
Dynatrace provides two types of access tokens. For this plugin, use the legacy Classic Tokens, which can be generated by searching for access token in the Dynatrace search bar. The newer Platform Tokens are the default type in Dynatrace, but this plugin is designed to work specifically with Classic Tokens.
...
For detailed instructions on generating these tokens, refer to the official [Dynatrace documentation](https://docs.dynatrace.com/docs/discover-dynatrace/references/dynatrace-api/basics/dynatrace-api-authentication#create-token).
