...
- Under the **Plugins** tab in **Configure**, go to `app-config.yaml` and **Edit YAML** to configure your Opsgenie instance in the **target** field and add your **domain** in the format as shown below.
...
The following configuration needs to set by users in **Layout** under **Configure** for **Sidenav** to get the opsgenie page. 
...
This plugin exports the above-mentioned card that you can show on the **Overview** tab of a service or any other layout page.  The following configuration is set by default in **Layout** under **Configure** for **Service** and you do not need to change anything:
