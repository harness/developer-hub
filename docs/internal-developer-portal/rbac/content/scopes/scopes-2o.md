...
To learn more about the entities, permissions, and different scopes we have introduced with IDP 2.0, refer to the [Data Model](/docs/internal-developer-portal/catalog/data-model) here. To learn more about configuring Harness RBAC, visit [Configure RBAC in Harness](/docs/platform/role-based-access-control/rbac-in-harness#configure-rbac-in-harness).
...
IDP 2.0 follows a three-level hierarchical structure. The three levels, or scopes, are **Account**, **Organization**, and **Project**. IDP resources can be explicitly created at any of these scopes. Learn more about the [Harness platform hierarchy](/docs/platform/get-started/key-concepts#account).
...
With IDP 2.0, you can create resources at any scope: **Account**, **Org**, or **Project**. Here is how resources, permissions, and scopes are linked in IDP 2.0:
...
IDP 2.0 utilizes Platform Hierarchy and RBAC to provide fine-grained control over IDP resources. There are three [key components](/docs/platform/role-based-access-control/rbac-in-harness#rbac-components) for Harness RBAC:
...
As IDP 2.0 introduces granular control over resources, you can assign [roles (with specific permissions) and resource groups (with specific access)](/docs/platform/role-based-access-control/rbac-in-harness#role-binding) to principals.
...
* [**Users**](/docs/platform/role-based-access-control/add-users): Individual users in Harness. A user can belong to multiple user groups. Roles and resource groups can be assigned directly to users, or inherited from their user groups. Learn more about [managing users](/docs/platform/role-based-access-control/add-users).

* [**User Groups**](/docs/platform/role-based-access-control/add-user-groups): Groups containing multiple users. Permissions and access applied to the group are inherited by all members. You can create user groups at any scope. Learn more about [creating user groups](/docs/platform/role-based-access-control/add-user-groups).
...
You can select which resources to include in a group based on the role you want to assign. Harness provides built-in resource groups at every scope, but you can also create custom ones. [Learn more about resource groups](/docs/platform/role-based-access-control/add-resource-groups).
...
Note: If a resource group is created at the **Project level**, you **cannot** select Org or Account scopes for it. Learn more about [Scopes and Refinement](/docs/platform/role-based-access-control/add-resource-groups#scopes-and-refinement).


### Roles

**Roles** are sets of [permissions](/docs/platform/role-based-access-control/permissions-reference) that allow or restrict specific operations on resources. They are applied **along with resource groups** to create effective RBAC policies.

Harness offers predefined roles, and you can also create [custom roles](/docs/platform/role-based-access-control/add-manage-roles#create-a-role) to enforce fine-grained access control. Roles are **scope-specific** and can be created at any scope.
...
2. [Create roles](/docs/platform/role-based-access-control/add-manage-roles) with appropriate permissions.
3. [Create resource groups](/docs/platform/role-based-access-control/add-resource-groups) for targeted access control.
4. [Create user groups](/docs/platform/role-based-access-control/add-user-groups) and [add users](/docs/platform/role-based-access-control/add-users).
5. [Assign roles and resource groups](/docs/platform/role-based-access-control/rbac-in-harness#role-binding) to the users or user groups.
6. [Configure authentication](/docs/platform/authentication/authentication-overview), if not already done.
