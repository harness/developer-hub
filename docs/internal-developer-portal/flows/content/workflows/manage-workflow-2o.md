...
### View Workflow YAML {#view-workflow-yaml}
...
#### Create Workflows with different branches {#create-workflows-with-different-branches}

You can also create Workflows with different branches and execute them separately. Whenever you're creating Workflows and storing their configuration in a third-party Git provider, or whenever you import a Workflow from an existing Git repository, you have the option to configure the **branch** of the repository from which you're creating the Workflow.

This means **different branches** of the same repository can be used to create different Workflows. You can also execute these Workflows from different branches. The **branch selector** feature enables you to select the branch from which you want to execute the Workflow.

Follow the given steps to create and execute Workflows with different branches:

1. Navigate to the **Workflows** page.
2. Click **Create**.
3. Select **Workflow** from the panel.
4. While configuring the Git connector, repository, branch, and YAML path, **select the branch** from which you want to create the Workflow.
5. Once all details are complete, click **Create Workflow** to finalize and register your Workflow in the catalog.
6. The Workflow will be created with the selected branch. You can execute this Workflow by going to the Workflow page and **switching the branch** from the branch selector. If a Workflow exists for that specific branch, it will appear and can be executed.
...
### Share Workflows {#share-workflows}
