...
This override behavior happens only at the display/rendering level, for example, when viewing the side panel or when components are used in Scorecards. However, this does not affect the actual YAML stored in Git, which remains unchanged unless manually edited. The flows are isolated: manual UI/YAML edits follow one path, and ingested data follows another. If there is a conflict, the ingested value is prioritized during rendering and internal evaluation.



### View ingested metadata
