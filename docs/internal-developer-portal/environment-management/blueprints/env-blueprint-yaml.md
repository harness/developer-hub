...
Platform engineers can use Harness OPA governance to enforce TTL requirements across all blueprints, e.g., blocking blueprints that use `ttl.kind: none` or that exceed a maximum TTL. These policies are evaluated automatically when a blueprint is created or updated. Go to [Centralized Policy Governance for IDP Catalog Entities](/docs/internal-developer-portal/governance/opa-governance) to enforce configuration standards on blueprints.
:::
