...
Follow the instructions below to ingest the SBOM for container images:
   ...
   * **Source:** Set the source, which can be Third party (Docker Registry, ECR, ACR, GAR) or Harness Artifact Registry. Depending on your selection, a unique set of fields will appear, each specific to the source you've chosen. Address these fields as required, this is similar to configuring the source in SBOM Orchestration step. Go to the [documentation on SBOM Orchestration](/docs/software-supply-chain-assurance/open-source-management/generate-sbom-for-repositories) to understand what each field entails. If you are using Docker Registry, you can follow along.
   * **Image:** Enter the name of your image with tag or digest, such as `my-docker-org/repo-name:tag` or `my-docker-org/repo-name@sha256:digest` .
   ...
   <DocImage path={require('./static/sbom-ingestion-steps.png')} width="100%" height="100%" />
...
<DocImage path={require('./static/ingest-sbom-repo.png')} width="50%" height="50%" />
...
* [Generate SBOM for repositories](/docs/software-supply-chain-assurance/open-source-management/generate-sbom-for-repositories): Generate SBOMs with Harness instead of ingesting them.