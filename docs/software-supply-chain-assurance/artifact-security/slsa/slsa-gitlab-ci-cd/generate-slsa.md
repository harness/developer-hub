...
3. [Configure SLSA provenance attestation in the workflow (optional)](#step-3-configure-provenance-attestation-optional)
...
Before configuring the SLSA Generation component, add the following variables to your GitLab project. Go to [Define a CI/CD Variable for a project](https://docs.gitlab.com/ci/variables/#for-a-project) to add them.
...
To attest the generated provenance, configure the attestation inputs (`ATTEST_WITH`, `OIDC_PROVIDER`, `FULCIO_URL`, `KMS_KEY`, and `VAULT_ADDR`) as described in [Step 3: Configure provenance attestation (optional)](#step-3-configure-provenance-attestation-optional).
...
### Step 3: Configure provenance attestation (optional)
...
To sign the generated SLSA provenance using a cryptographic key, set `ATTEST_WITH` to `keybased` and provide the Cosign key material as masked GitLab CI/CD variables on the job. `KMS_KEY` is not used for key-based attestation. It applies only to Secret Manager attestation.
...
* [Verify SLSA with GitLab CI/CD](/docs/software-supply-chain-assurance/artifact-security/slsa/slsa-gitlab-ci-cd/verify-slsa): Verify SLSA provenance and validate the integrity of software artifacts before promotion or deployment.
