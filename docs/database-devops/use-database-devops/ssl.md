...
* for Git Clone step, certificate is generated and mounted at `/etc/ssl/certs/ca-bundle.crt`
  * to know more, refer the info section in [git clone step ssl](https://developer.harness.io/docs/continuous-integration/use-ci/set-up-build-infrastructure/k8s-build-infrastructure/configure-a-kubernetes-build-farm-to-use-self-signed-certificates/)
...
In order for the JDBC test connection to work, 

in env variable `INIT_SCRIPT` using `client.key` and `client.crt` `client.p12` key is generated imported to the java key store

Below is an example which generates pkcs12 file and imports to the **default key store** `$JAVA_HOME/lib/security/jssecacerts`
...
:::info note
* This `INIT_SCRIPT` section is needed since the JDBC Test Connection runs within a delegate
...
To override, [override trust store for delegates](https://developer.harness.io/docs/platform/delegates/secure-delegates/trust-store-override-for-delegates/)
