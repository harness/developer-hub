...
For Self-Managed Enterprise Edition installations, the `ET_COLLECTOR_URL` will be dependent on the [Load Balancer URL](/docs/self-managed-enterprise-edition/self-managed-helm-based-install/install-harness-self-managed-enterprise-edition-using-helm-ga.md#add-a-load-balancer) for example a `loadbalancerURL: https://example.harness.io/` would require `ET_COLLECTOR_URL=https://example.harness.io/et-collector`
