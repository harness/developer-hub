...
The Agent can be installed on a variety of operating systems. The operating systems must be 64-bit for compatibility with the CET Agent. For example, _x86-64_, _AArch64_ and _PowerPC 64_.
...
When using non-Java languages with the CET Agent, use the source attach to fully experience the user interface. This is because if the source files are missing, CET uses decompiled sources to display your code. Note that decompilers only support Java, and if code generated by non-Java languages is decompiled, it may lose readability and become unusable.
