...
3. If you select **OIDC** as the credential type, you can use delegate selectors in OIDC tokens to enforce AWS IAM policies based on which delegates execute tasks. For details, go to [Delegate selectors as session tags](/docs/platform/connectors/cloud-providers/ref-cloud-providers/aws-connector-settings-reference#delegate-selectors-as-session-tags).
