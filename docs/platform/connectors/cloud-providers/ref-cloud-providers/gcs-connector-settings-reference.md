...
To connect to GCP with OIDC, you must configure an [OIDC identity provider](https://cloud.google.com/identity-platform/docs/web/oidc#configuring_the_provider) in GCP and add it in a trust relationship with an IAM role you create that Harness will use to operate in GCP. Use the following Harness OIDC provider endpoint and OIDC audience settings to create your OIDC identity provider.

   * Harness OIDC provider endpoint: `https://app.harness.io/ng/api/oidc/account/<YOUR_ACCOUNT_ID>`

   * OIDC audience: `sts.amazonaws.com](https://iam.googleapis.com/projects/<YOUR_GCP_PROJECT_ID>/locations/global/workloadIdentityPools/<YOUR_WORKLOAD_POOL_ID>/providers/<YOUR_PROVIDER_ID>`
