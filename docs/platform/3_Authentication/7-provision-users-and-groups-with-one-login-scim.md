...
The provisioned user cannot use the Harness OneLogin app to log into Harness unless OneLogin is also set up for [OneLogin SAML authentication in Harness](../3_Authentication/3-single-sign-on-saml.md#saml-sso-with-onelogin). They must use their email address and password.
...
1. In Harness Manager, create an API token by following the instructions in [Add and Manage API Keys](../4_Role-Based-Access-Control/5-add-and-manage-api-keys.md).
