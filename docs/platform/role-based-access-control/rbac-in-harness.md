...
With the Secret permission split, you can now manage create and edit actions separately, giving you more granular control over secret management.

#### Feature Flag Rollout Process

This feature is controlled by two feature flags that must be enabled in the following order:

- `PL_SECRET_CREATE_EDIT_PERMISSION_SPLIT_MIGRATION`: This flag enables migration — roles are migrated into two separate permissions. It adds the `create` permission to all secrets that already have `edit`.
- `PL_SECRET_CREATE_EDIT_PERMISSION_SPLIT_ENFORCE`: Permissions are now enforced — UI changes and access checks depend on the split permissions: `create` and `edit`. 
...
<details>
<summary>View all Secrets permission</summary>
...
**Without feature flags:**
- `core_secret_edit` — combined permission to create and edit secret  

</details>

:::warning 
Once both feature flags are enabled, please review your automations if they rely on `core_secret_edit` for both creating and editing secrets. With the update, `core_secret_edit` will only allow editing, and creating secrets will require `core_secret_create`. 

To avoid any disruption, ensure your automations are updated to include `core_secret_create` alongside `core_secret_edit` where necessary.
:::
