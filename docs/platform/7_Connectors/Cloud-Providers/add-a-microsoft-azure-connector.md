...
If you connect to an ACR repo via the platform-agnostic [Docker Connector](../Cloud-Providers/references/docker-registry-connector-settings-reference.md), the limit is 100.
...
* Use the platform-agnostic [Kubernetes cluster connector](../Cloud-Providers//add-a-kubernetes-cluster-connector.md) with a Kubernetes or Helm delegate.
...
For more information, go to the **Deployments (CD)** section of the [Kubernetes cluster connector settings reference](../Cloud-Providers/references//kubernetes-cluster-connector-settings-reference.md).
...
5. Provide an authentication key for your app. For **Authentication**, select either **Secret** or **Certificate**, and then select or create a [Harness Text Secret](../../6_Secrets/2-add-use-text-secrets.md) or [Harness File Secret](../../6_Secrets/3-add-file-secrets.md).
