...
The `-H 'X-Api-Key: sample_api_key'` parameter is used to authorize custom triggers. You can use [Harness API keys](../User-Management/7-add-and-manage-api-keys.md) with this parameter. This is described below in [Custom trigger authorization using API keys](#custom-trigger-authorization-using-api-keys).
...
You can use [Harness API keys](../User-Management/7-add-and-manage-api-keys.md) in your cURL command to authorize the execution of a trigger.
...
1. Create the [Harness API key](../User-Management/7-add-and-manage-api-keys.md) you want to use for authorization.
