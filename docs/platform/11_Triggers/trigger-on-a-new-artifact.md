...
* Due to a Docker API limitation, image build numbers/tags are always listed in lexical order. To ensure executions are triggered with the image pushed last, a best practice is to create build numbers or tags that can be sorted lexically using their creation date. Using this method, higher build numbers are assigned for for higher creation dates. This will ensure that the image pushed last is used when more than one image is pushed over a short period of time, such as less than 5 minutes.
