...
3. For **GitLab** the search functionality will only work with the repository names, not the full path or subdirectory. Additionally, search will not work for cases like `subgroup` or `project`.
