...
| **Retry** | Harness retries the execution of the failed step automatically. You can set **Retry Count** and **Retry Intervals**. | Harness can also retry the execution of the complete step group automatically, from the beginning. You can set **Retry Count** and **Retry Intervals**. | Same as step. |
...
:::info note Currently, the Failure Strategy of Retrying all steps of a Step Group is behind the Feature Flag `PIE_RETRY_STEP_GROUP`. Contact [Harness Support](mailto:support@harness.io) to enable the feature.
