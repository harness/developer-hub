...
* **Regex** - Expects a single Regex value, which can be a complex Regex expression, such as `^((?!README\.md).)*$`. Matches full paths based on the Regex. Use this operator if you want to specify multiple paths.
