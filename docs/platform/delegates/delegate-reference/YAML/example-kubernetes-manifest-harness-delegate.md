...
```yaml
# Definition of a Kubernetes Namespace for the Harness Delegate
apiVersion: v1
kind: Namespace
metadata:
...
# ClusterRoleBinding to grant cluster-admin privileges to the default ServiceAccount within the harness-delegate-ng Namespace
...
# Secret to store the token for the Harness Delegate
apiVersion: v1
kind: Secret
metadata:
...
# Deployment configuration for the Harness Delegate
apiVersion: apps/v1
kind: Deployment
metadata:
  labels:
...
# HorizontalPodAutoscaler configuration for the Harness Delegate
...
# Role for managing CronJobs and Deployments within the harness-delegate-ng Namespace
kind: Role
apiVersion: rbac.authorization.k8s.io/v1
metadata:
...
# RoleBinding to bind the upgrader-cronjob Role to the upgrader-cronjob-sa ServiceAccount
kind: RoleBinding
apiVersion: rbac.authorization.k8s.io/v1
metadata:
  name: upgrader-cronjob
...
# ServiceAccount for the upgrader-cronjob
apiVersion: v1
kind: ServiceAccount
metadata:
  name: upgrader-cronjob-sa
...
# Secret to store the token for the upgrader CronJob
apiVersion: v1
kind: Secret
metadata:
...
# ConfigMap containing configuration for the upgrader CronJob
apiVersion: v1
kind: ConfigMap
metadata:
...
# CronJob to periodically trigger the upgrader process
apiVersion: batch/v1
kind: CronJob
metadata:
  labels:
