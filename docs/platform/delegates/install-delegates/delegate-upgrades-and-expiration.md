...
:::info
The automatic upgrade feature is enabled by default for the Kubernetes manifest and Helm installation options. However, the automatic upgrade feature is not supported for the Terraform and Docker installation options and is disabled by default.
:::
