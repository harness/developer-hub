...
When you use multiple delegate tags in the Delegate Selectors setting of a step, Harness selects only the delegate(s) that have all the selectors.

This means that if there are multiple tags, the delegate must match all of them to be selected. For instance, if you have a delegate with three tags and you only provide two of them in the Delegate Selectors setting, the delegate is still eligible for selection as long as the two tags are present for that delegate.
