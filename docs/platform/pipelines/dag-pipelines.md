---
title: DAG pipelines
sidebar_label: DAG Pipelines
description: Learn how to configure pipelines using Directed Acyclic Graph (DAG) dependencies.
sidebar_position: 11
keywords:
  - DAG pipelines
...
:::note
Currently, this feature is behind the feature flag `PIPE_ENABLE_DEPENDENCY_BASED_EXECUTION`. Contact [Harness Support](mailto:support@harness.io) to enable the feature.
:::
...
DAG pipelines are ideal for workflows where different stages have specific dependencies that don't fit into a simple sequential or parallel pattern.
...
The pipeline editor opens in DAG mode. Unlike traditional pipelines, DAG pipelines don't show start or end nodes in the visual graph.
...
When you select a stage in the pipeline visual editor, you'll see three action buttons that help you configure dependencies:
...
The dropdown shows only stages that won't create a circular dependency. Harness automatically excludes:
...
- Deploy Frontend doesn't wait for Integration Tests (selective independence)
...
A stage begins executing as soon as all of its dependencies complete successfully. Harness doesn't wait for other stages that aren't dependencies. This "execute as soon as possible" approach maximizes parallelism and reduces overall pipeline execution time.
...
- Stage A doesn't wait for D or E to complete
...
- If any dependency fails, Stage X won't execute (unless you configure failure strategies)
...
DAG pipelines can have completely independent execution paths. Stages in one path don't affect stages in another path unless they share dependencies.
...
When you use stage templates within a DAG pipeline, the template itself doesn't define dependencies. Dependencies are configured at the pipeline level, where you add the stage template to the pipeline.
