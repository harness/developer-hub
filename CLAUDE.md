...
- Structure: Frontmatter → Introduction → What you will learn from this topic → Optional lightweight prerequisites → Concept sections → Optional FAQs → Related concepts
...
- Deliberately **omit** "What you will learn from this topic", `<Troubleshoot>` blocks, concept dumps (hierarchies, version matrices, "how RBAC works"), and standalone "Limitations" sections. Link concept and platform-level material out, or surface it under Next steps. Score these pages against this rule, not the feature or overview rubric.
...
## 3k-docs Content Reuse

`3k-docs/` has historically been a parallel tree of thin wrapper pages that do not contain real content. **This is changing module by module:** some modules are being cleaned out and rewritten with real, hand-authored content directly in `3k-docs/` instead (Continuous Delivery first, then Harness AI, Platform, and FME are planned). Check whether a given `3k-docs/` file has an `@site/...` import before assuming the reuse rule below applies to it. Go to `.cursor/rules/3k-docs.mdc`'s "Wrapper pages vs. rewritten modules" section for the full breakdown, including a known gap: `redirect_from` frontmatter is currently only honored for `docs/` pages, not `3k-docs/` pages, so internal moves within a rewritten module need `_server-redirects` instead until that scanner is generalized.

The legacy wrapper pattern: each page imports and renders a component from `docs/` (or occasionally `src/components/`):

```mdx
---
title: CI Supported Platforms
---

import Ci from '@site/docs/continuous-integration/ci-supported-platforms.md';

<Ci />
```

Frontmatter is owned independently by the `3k-docs/` page; only the body is reused via the import. `scripts/convert-3k-docs.mjs` generates this wrapper pattern for a module.

**Rule:** Whenever you move, rename, or delete a file under `docs/` or `src/components/`, check `3k-docs/` for importers of the old path (`grep -rn "@site/docs/<old-path>" 3k-docs/`, dropping the extension for component paths) and fix the import in the same change:

- **Clean rename/move:** Rewrite the import statement to the new path. Nothing else in the wrapper changes.
- **Split, merged, or restructured content:** There is no mechanical fix. Point the wrapper at the correct source(s), add a second import if the content now lives in two files, or regenerate the wrapper for the whole module with `node scripts/convert-3k-docs.mjs <module-path> <component-name>`.
- **File deleted outright:** Decide whether the `3k-docs/` wrapper should be deleted, inlined with real content, or repointed. Never leave a dangling import.

**Why this matters:** An `@site/docs/...` or `@site/src/...` import pointing at a path that no longer exists fails `docusaurus build` (`onBrokenLinks: 'throw'`) with a module-not-found error. This is exactly what caused the HDH-561 incident: a docs/ page move shipped without updating the matching `3k-docs/` import, the build broke on merge to `main`, and every subsequent PR's preview build failed until the stale imports were traced by hand.

**`sidebars-3k-docs.ts` has the same manual/autogenerated duality as `sidebars.ts`.** Go to **Sidebar Navigation Configuration** above for the full System 1 / System 2 explanation; the same pattern repeats here for the `3k-docs/` tree:

- **Manual doc-ID references:** `link: { type: 'doc', id: 'platform/getting-started/index' }` — `id` is a path under `3k-docs/`, no extension.
- **Autogenerated category references:** `items: [{ type: 'autogenerated', dirName: 'code-repository/config-repos' }]` — `dirName` is a folder path under `3k-docs/`.

Whenever you move, rename, or delete a file or folder under `3k-docs/` itself, grep `sidebars-3k-docs.ts` for `id: '<old-path>'` and `dirName: '<old-folder>'` and update every match in the same change. A stale `id` or `dirName` fails the build the same way a stale import does, just through the sidebar plugin instead of webpack.

`scripts/check-3k-docs-drift.mjs` runs as a CI step (`3k-docs Drift Check (warn-only trial)` in `.harness/pipelines/harnessDeveloperHub3k.yaml`, an isolated trial pipeline — not yet wired into the main HDH build/preview pipeline) as a backstop — it auto-fixes clean renames (both `@site/...` imports and `sidebars-3k-docs.ts` `id:` entries, skipping anything inside a commented-out block) and fails the build on anything ambiguous, including `dirName:` folder renames, which always need a human. Treat it as a safety net, not the first line of defense: fixing the import or sidebar reference in the same PR that moves the file means the check passes silently instead of blocking the build.

**Content-parity reminders (non-blocking):** `3k-docs-module-status.json` at the repo root lists which top-level modules are `rewritten`. For those modules, the same drift script also checks whether a changed `docs/<module>/X.md` has a `3k-docs/<module>/X.md` counterpart that wasn't touched in the same PR, and prints a reminder if so — never a failure, since divergence is expected once a module is rewritten. Add a module to the `rewritten` array when you cut it over. Go to `.cursor/rules/3k-docs.mdc`'s "Content-parity reminders for rewritten modules" section for details.

Full conventions: `.cursor/rules/3k-docs.mdc`.
