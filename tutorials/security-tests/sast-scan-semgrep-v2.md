---
title: Scan a codebase (SAST scans)
sidebar_position: 5
...
- This tutorial has the following prerequisites:
  ...
  - Your Git access token must be stored as a [Harness secret](/docs/platform/secrets/add-use-text-secrets)
  - This tutorial illustrates a basic setup workflow using two popular scanners: Bandit and Semgrep. Both scanners have free versions available. 
  
    - Bandit is simpler to set up, but supports Python only.
    - Semgrep supports a [wide variety of languages](https://semgrep.dev/docs/supported-languages/), but also requires the following:
      ...
      - Your Semgrep access token must also be stored as a [Harness secret](/docs/platform/secrets/add-use-text-secrets)
      - You need to add a Run step to run the Semgrep scan and save the results. 
...
:::note
This step is required for Semgrep scans only. If you're running a Bandit scan, proceed to the [next step](#ingest-the-scan-results).
:::
